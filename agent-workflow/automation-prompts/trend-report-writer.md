...
基于趋势候选、变化候选 / 成熟变化短专题、场景 / 案例信号、前沿观点、急件候选和来源数据，写趋势追踪报告，并维护趋势候选生命周期。
...
- `01-SiteV2/content/11-databases/urgent-trend-candidates/README.md`
...
1. 产出 1 篇 Trend Report，并按需要更新趋势候选、Publication Index 和关联资产。
2. 产出 1 份 `no_report_decision`，说明为什么本次不写报告。

不能硬写，也不能沉默。不能用简短运行报告替代 `no_report_decision`。
...
```text
01-SiteV2/content/08-trend-reports/flash/
```

深度报告：

```text
01-SiteV2/content/08-trend-reports/full/
```
...
不要把新报告写入 `08-trend-reports/` 根目录；新报告只进入 `flash/`、`full/` 或 `no-report-decisions/`。
...
- 趋势追踪不是卡片聚合页，而是深度长文报告 + 趋势候选 / 趋势状态记录。
- 报告要像研究报告，不是结构化填空。
...
趋势报告必须回看所引用商业信号、成熟变化短专题、场景 / 案例信号、前沿观点背后的 Raw 证据对象。满足以下条件的 Raw 才能作为主证据：
...
- 维护 `trend_evidence_gate`，不得用过时的 `source_level / data_level / credibility_boundary` 作为趋势准入判断。
...
- 未完成 Raw 回填的商业信号、变化候选或场景候选只能作为背景，不能作为趋势成立主证据。
- 前沿观点只能说明行业预期、叙事变化、产品路线或资本关注点；观点里的事实主张必须另补 S/A/B 来源。
- 前沿观点必须来自已完成中文翻译和四档评级的观点卡；`feature` / `sidebar` 可作为前台参照，`archive` 仅作内部校准，`discard` 不得引用。
- 趋势候选后台人读文案和前台展示文案必须遵守 `agent-workflow/product/card-asset-copy-governance.md`：先写底层变化，再写阶段性判断，再写风险边界和证据缺口。
- 趋势候选必须区分 `backend_prose` 与 `frontend`。后台可保留趋势门槛、来源计数和证据缺口；前台不得显示 Raw / Pool / gate / threshold / 补证等内部生产语言。
- Raw 原文、source quote、观点原文、关键摘录、来源链接和证据快照不得为了文风改写。趋势报告和趋势候选只能新增解释性文字，不能改写证据。
- 只有趋势候选生成文案通过 `guanlan-copy-style-qc` 和 `cardcopy` 硬闸门，才能写 `frontend_copy_gate: passed` 与 `cardcopy_gate: passed`。
