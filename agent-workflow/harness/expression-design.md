...
这是观澜 AI 项目内的使用导航。共享规范的唯一维护源已集中到私有统一管理仓 `Harness/`；下列规范文档为自动同步的运行副本，映射见 [shared-sources.json](shared-sources.json)。改规范先改维护源并同步；品牌资产、Skill 程序与项目页面契约继续在本仓维护。管理台对应 `H-expression-design`，并将共享文件定位到维护源。
...
- 修改共享规范：编辑私有管理仓 Harness 中对应维护源，经 shared.py 同步后更新运行副本；项目专属契约和资产在本仓修改。改变路径时同步 `manifest.json` 与本导航，然后刷新管理台。新增需求或验收缺口沿用既有任务，不新增重复任务队列。
