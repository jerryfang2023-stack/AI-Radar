...
`clean_text` 不能补位为 `full_text`。入口摘要、搜索摘要、AI HOT 摘要和 follow-builders 转述不得作为事实依据。
...
- AI HOT daily 可提高 Raw 抓取优先级和候选保留优先级。
...
- AI HOT daily selected 全量保留但未通过证据门的项目。
...
- AI HOT daily selected 为全量保留要求可降到 `index_only`，但不得因精选身份避免事实使用阻断。
...
- `pool_index_count`：Pool index 总量，包含 AI HOT daily selected 的全量保留。
...
- `aihot_index_only_count`：AI HOT selected 中只索引的项。
- `aihot_core_count`：AI HOT selected 中真正过证据门进入核心池的项。

质量判断不得只看 `pool_count`，因为 AI HOT 全量保留会抬高 Pool index 数量。
