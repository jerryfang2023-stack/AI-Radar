...
趋势追踪不是趋势名称列表，也不是公司案例库。它判断一组 AI 商业变化是否正在连成一个方向：谁会受影响，谁可能付钱，证据够不够，反证在哪里。

趋势体系分两层：

| 资产 | 角色 | 前台形态 |
|---|---|---|
| Trend Card | 趋势状态记录 | 卡片、关系网络、筛选和后台状态 |
| Trend Report | 趋势判断文章 | 深度长文、趋势快报、报告详情页 |

`Trend Card` 负责记住趋势状态，`Trend Report` 负责形成判断。老板不会因为一张趋势卡产生认同，但系统需要趋势卡来记录 7 / 30 / 90 天的变化。
...
- 趋势追踪是 `长文报告 + 趋势卡 / 变化簇` 双重结构。
- 每个栏目仍以文章阅读为主，不把前台做成后台结构化填充页。
- 今日观察是当天市场综述，不由少量变化卡样本直接推导全局判断。
- 商业内参是周期刊物，不照搬每日观察或单篇趋势报告。
- 自动化保持六线程，不新增第七个线程。
- 所有来源、事实、数据、观点和反证必须标注来源名、来源等级、原始链接和增量事实。
...
正式 Trend Report 是趋势追踪的核心付费资产，必须有足够证据、反证、案例、竞品、客户场景、技术路线和商业模式分析。

## 4. 正式 Trend Report 生成条件

正式报告不能每天硬写，也不能等到完全确定才写。最低门槛是“三类证据 + 一个反证”：

1. 至少 3 条独立商业信号，不能来自同一个媒体转述链。
2. 至少 1 个具体案例：公司、产品、客户采用、融资、招投标、收入或合作。
3. 至少 1 条外部观点或技术路线材料，用来解释为什么现在发生。
4. 至少 1 个反证或不确定变量，说明为什么现在还不能下结论。
...
没有反证，不允许成为正式 Trend Report。没有反证的报告容易变成宣传稿。
...
1. 系统自动复核：检查多源、来源等级、商业影响、同类案例、竞品动作、反证和不确定项。
2. 人工批量复核：每天或每半天看一次急件候选清单，只做 `放行临时报告 / 继续观察 / 降级为趋势卡`。
...
```text
01-SiteV2/content/09-databases/urgent-trend-candidates/YYYY-MM-DD-urgent-trend-candidates.md
```
...
6. 反证与观察变量：哪些东西还没有被证明。
7. 30 天后看什么：是否升级正式报告的追踪钩子。

### 6.4 文案风格

趋势快报必须遵守 `COPY.md` 和 `skills/guanlan-writing-style/SKILL.md`。它不能写成新闻快讯、自媒体爆款或技术圈热闹总结。
...
Trend Card 和 Trend Report 都可引用趋势状态，但前台要转译成人话。
...
- 7 / 30 / 90 天观察变量。
- 来源与事实。
- 相关 Signals / Trend Cards / Case Cards / Opinion Cards。
- 机会判断段落。
- 反证与边界。
...
1. 优先读 `full/`。
2. 再读 `flash/`。
3. 兼容读 `06-trend-reports/` 根目录旧文件。
4. 不读取 `no-report-decisions/` 入前台。

生产写新结构，读取短期保留兼容。兼容期结束后再清理根目录旧文件。
