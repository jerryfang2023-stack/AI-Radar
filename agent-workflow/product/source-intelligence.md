...
AI HOT daily 和 follow-builders 是精选入口，可以给更高采集权重；这个权重来自“精选入口”，不来自 `S/A/B/C/D/M`。采集入口只影响发现和抓取顺序，不决定 `core_pool`、`watchlist` 或 `index_only` 路由。
...
AI HOT daily 是每日精选，必须完整保留，并给更高采集权重。它既不自动进入 `core_pool`，也不自动降级为 `index_only` / `watchlist`；必须回到原文后按统一 Raw / Pool / Card 证据门槛判定。
...
- 不使用 AI HOT、RSS、搜索、GDELT、Anysearch、Tavily、Exa 等采集入口做自动升降级；候选必须回到原文后再判断是否进入 `core_pool`。
