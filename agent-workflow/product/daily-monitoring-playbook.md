...
- AI HOT daily selected 全量保留用于审计和补采，但必须回到原文后按统一证据门槛判定；不得自动进入 `core_pool`，也不得因渠道身份自动降级到 `index_only` / `watchlist`。
...
- `raw_qc_decision=block`：`discard`；精选入口项可保留为 `index_only` 审计记录，但仍不得下游使用。
