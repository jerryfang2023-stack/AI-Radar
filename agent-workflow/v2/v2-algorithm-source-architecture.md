...
> 2026-05-10 运行口径更新：本文中早期 `Raw Candidate 30-50` 是 V2-2 架构阶段基线，不再作为当前每日生产自动化目标。当前 `V2-DAILY-AUTO / v2-content-site-daily-update` 已升级为 Raw 80-150、Pool 20-30、Structured 8-15、Front 3-5、Deep Dive 1-2、Trend Updates 3-5；AI HOT / follow-builders 等只作为 M 级 discovery / source-router，必须解析原始 URL 后按 S/A/B/C/D 分级。
...
目标：每日 80-150 条；低信号或关键接口失败日可降级为 50-80。
...
2026-05-10 注：用户已放弃 7 日隔离验证并进入 V2-only 生产线；以下 30-50 数量为历史验证计划，不是当前每日自动化生产目标。当前每日生产目标以本文顶部 2026-05-10 运行口径更新为准。

### 12.1 每日验证

每日产出：

- Raw 80-150；低信号或接口失败日可降级 50-80 并说明原因。
- Pool 20-30。
- Structured 8-15。
- Front Signal 3-5。
- Trend update 3-5。
...
Deep Dive 只在证据充足时产出 1-2 篇；证据不足时不得硬凑。
