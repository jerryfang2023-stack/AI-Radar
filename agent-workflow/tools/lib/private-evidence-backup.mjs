...
export function buildPrivateEvidenceBackup({
  ...
  if (!fs.existsSync(originalsRoot)) {
    throw new Error(`Repository original snapshot source is missing: ${originalsRoot}`);
  }
  ...
  for (const jsonFile of listFiles(originalsRoot, ".json")) {
    ...
    const entry = {
      ...
      markdown_snapshot_ref: fs.existsSync(jsonFile.replace(/\.json$/iu, ".md"))
    ...
    if (
      ...
      historical.push({
        ...entry,
        migration_status: "isolated_private_backup",
  ...
  writeLines(path.join(resolvedBackupRoot, "catalog.jsonl"), catalog);
  writeLines(
    path.join(resolvedBackupRoot, "manifests/non-production-historical-sources.jsonl"),
    historical,
  );
  ...
  const effectiveGeneratedAt = previousManifest.catalogDigest === catalogDigest
    ...
    : generatedAt;
  const summary = {
    schemaVersion: "PRIVATE-EVIDENCE-BACKUP-V1.0",
    generatedAt: effectiveGeneratedAt,
    catalogDigest,
    sourceRoot: "01-SiteV2/content/01-raw/originals",
    productionReferenceDate: production.date,
    objectPolicy: "one_body_per_content_hash",
    discoveryPolicy: "backup_is_outside_repository_and_v4_discovery",
