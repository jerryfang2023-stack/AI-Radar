...
function promptFor(event, company, sources, directions) {
  ...
  return [
    ...
    "analysis.product_form_id必须选择公司主要面向客户或用户提供的一种核心产品形态。先判断客户实际购买或用户直接使用什么，再判断交付界面；不得因为产品采用某种模型、芯片、机器人或安全技术，或者计划进入某个行业，就把底层技术或未来场景当成主分类。允许值：model（模型能力本身）、api_service（API/托管模型端点）、developer_tool（开发生命周期工具）、ai_application（直接完成明确任务的成品应用，含企业垂直应用）、enterprise_platform（覆盖多流程的企业平台/操作系统/控制平面）、data_infrastructure（训练/推理/记忆/路由/压缩等软件基础设施）、security_product（安全本身是核心产品）、ai_device（用户直接使用的AI硬件）、robot（作用于物理世界的机器人）、chip_accelerator（芯片/加速器/IP/关键部件）、compute_system（服务器/机架/集群/数据中心/网络等实体系统）、compute_service（云端GPU/训练/推理容量服务）。",
    "analysis.market_category_id按CB Insights AI 100的核心供给框架三选一：ai_infrastructure（向其他AI产品提供计算、芯片、数据、模型、训练、运行、安全或编排底座）、horizontal_ai（跨行业复用的通用职能或大众任务产品）、vertical_ai（围绕一个明确行业的专业数据、流程、监管或物理环境构建）。不要用企业/消费者、软件/硬件或API/平台这些交付形式代替市场类别。",
    ...
    JSON.stringify({
      ...
      analysis: {
        ...
        product_form_id: "model|api_service|developer_tool|ai_application|enterprise_platform|data_infrastructure|security_product|ai_device|robot|chip_accelerator|compute_system|compute_service",
        market_category_id: "ai_infrastructure|horizontal_ai|vertical_ai",
        sector: "string",
      },
    }),
