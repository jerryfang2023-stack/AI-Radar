...
const GENERIC_ROUNDUP_TITLE = /^(?:AI\s+giants?|AI\s+companies?|AI\s+startups?)\b.{0,100}\b(?:billions?|millions?|funding|investment|deployment)\b|^AI\s*(?:巨头|公司|初创企业).{0,80}(?:数十亿|数百万|融资|投资|部署)|^latest\s+open\s+(?:models?|artifacts?)\s*(?:\(#?\d+\)|#\d+)?\s*[:：]|^最新(?:开源)?(?:模型|产品|模型与产品)?(?:盘点|汇总)\s*(?:[（(]#?\d+[）)])?\s*[:：]|\b(?:daily|weekly)\s+(?:AI\s+)?(?:roundup|digest)\b|(?:每日|每周|本周).{0,20}(?:汇总|速览|快讯)/iu;
