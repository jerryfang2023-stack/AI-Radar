...
const schemaPath = path.join(root, "agent-workflow/product/entity-catalog-review-v1.schema.json");
...
function main() {
  ...
  for (const decision of decisions) {
    ...
    if (decision.action === "confirm" && decision.error_patterns?.length) problems.push(`confirmed entity must not carry an error pattern: ${decision.entity_id}`);
    if (decision.action !== "confirm" && !decision.error_patterns?.length) problems.push(`non-confirm decision requires an error pattern: ${decision.entity_id}`);
    for (const pattern of decision.error_patterns || []) if (!ENTITY_REVIEW_ERROR_PATTERNS.includes(pattern)) problems.push(`unknown error pattern: ${decision.entity_id}:${pattern}`);
  ...
  console.log(JSON.stringify({
    ...
    error_patterns: Object.fromEntries(ENTITY_REVIEW_ERROR_PATTERNS.map((pattern) => [pattern, decisions.filter((decision) => decision.error_patterns?.includes(pattern)).length]))
  }, null, 2));
}
