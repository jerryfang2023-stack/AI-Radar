...
function datedFiles(name) {
  return fs.readdirSync(dataCenterRoot, { withFileTypes: true })
    .filter((entry) => entry.isDirectory() && /^\d{4}-\d{2}-\d{2}$/u.test(entry.name))
    .sort((a, b) => a.name.localeCompare(b.name))
    .map((entry) => path.join(dataCenterRoot, entry.name, name))
...
export function taxonomyConsistencyProblems(rootDir = root) {
  ...
  const ledger = readJson(path.join(fundingRoot, "taxonomy-decisions-v4-1.json"), { decisions: [] });
  const reviewed = datedFiles("reviewed-event-classifications.json").flatMap((file) => readJson(file, []));
  ...
  const claims = new Map(datedFiles("claims.json").flatMap((file) => readJson(file, [])).map((item) => [item.claim_id, item]));
  for (const name of ["tag-assertions.json", "facet-assertions.json"]) {
    for (const assertion of datedFiles(name).flatMap((file) => readJson(file, []))) {
      if (assertion.status && assertion.status !== "active") continue;
      const claim = claims.get(assertion.evidence_ref || assertion.asset_id);
      if (claim && !taxonomyEvidenceSegmentRelevant(claim)) {
        failures.push(`${assertion.assertion_id}: secondary-party or biography evidence cannot assign target taxonomy`);
      }
    }
  }
  ...
  for (const record of opportunity.evidence || []) {
    ...
    for (const item of record.classifications || []) assertKnownClassification(item, validValues, `opportunity ${record.event_id}`, failures);
  }
