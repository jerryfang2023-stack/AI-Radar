...
import { isEligibleFundingInsightEvent, normalizeFundingRound } from "./funding-insight-v1-utils.mjs";
...
export function historyCaseClosure(ledger, { raws = [], claims = [], events = [], cards = [], outcomes = [], queue = [] } = {}) {
  ...
  return ledger.cases.map((row) => {
    ...
    const readable = originals.filter((raw) => ["accepted", "partial"].includes(raw.extraction_status) && raw.body_length > 0
      && outcomeByRaw.get(raw.raw_id)?.reason !== "raw_source_quality_block");
    const name = normalized(row.company_hint);
    const stem = name.replace(/(?:科技|studio)$/u, "");
    const subject = stem.length >= 3 ? stem : name;
    const sourceMatches = events.filter((event) => event.event_type === "funding" && event.publication_status === "verified"
    ...
    const hintRound = normalizeFundingRound(row.round_hint).code;
    const conflicts = sourceMatches.filter((event) => {
      const actual = normalizeFundingRound(event.object).code;
      return !["undisclosed", "other", "multi_round"].includes(hintRound) && !["undisclosed", "other", "multi_round"].includes(actual) && hintRound !== actual;
    });
    const matches = sourceMatches.filter((event) => !conflicts.includes(event));
    ...
    const status = exactCards.length && chinaEvents.length ? "card_available"
      ...
      : matches.length ? "market_verification_pending" : conflicts.length ? "index_round_conflict" : !row.original_candidates.length ? "original_source_unresolved"
        : readable.length ? "fact_verification_pending" : originals.length ? "original_unreadable_after_capture" : "original_capture_pending";
    ...
    return { case_id: row.case_id, company_hint: row.company_hint, date_hint: row.date_hint, round_hint: row.round_hint, status,
      original_candidate_count: row.original_candidates.length, captured_originals: readable.length, raw_snapshot_count: originals.length,
      readable_originals: readable.length,
      ...
      round_conflict_event_ids: conflicts.map((event) => event.event_id),
