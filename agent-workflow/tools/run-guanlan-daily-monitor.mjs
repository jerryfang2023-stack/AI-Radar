...
function importanceProfile(item = {}, bodyText = "") {
  ...
  const marketStructureEvent =
    /\b(acquires?|acquired|acquisition|merger|buyout|strategic partnership|partners? with|collaborates? with|procurement|tender|rfp|contract|lease|leasing|agreement|capacity deal|pricing|price increase|price cut|rate limit|billing change|regulatory approval|clearance|antitrust|lawsuit|settlement)\b|收购|并购|合并|战略合作|合作伙伴|采购|招标|投标|合同|签约|租赁|协议|容量|园区|负载|装机|定价|价格|计费|监管批准|审批|反垄断|诉讼|和解/iu.test(text);
  ...
  if (marketStructureEvent && marketStructureBusinessContext && !marketStructureNoise) {
    add("important_market_structure", /\b(acquisition|merger|contract|lease|leasing|agreement|capacity deal|procurement|tender|regulatory approval|lawsuit|settlement)\b|收购|并购|合同|租赁|协议|容量|园区|负载|采购|招标|监管批准|诉讼|和解/iu.test(text) ? 5 : 4, "market-structure commercial event");
  }
...
function poolRoutesFor(item, quality, scores, usable, excerpts = [], rawQcDecision = "") {
  ...
  // Discovery provenance must not permanently demote an article once the
  // monitor has resolved a readable original page for a concrete commercial
  // event. This is how a HN/AI-HOT lead such as a production deployment, or
  // a first-party pricing release, returns to the same Core review path as a
  // directly discovered original source.
...
async function runEvidenceObjectRegressionFixtures() {
  ...
  const recoveredMeta = poolCandidateMeta(recoveredOriginalEvent);
  const recoveredRoutes = recoveredMeta.routes;
  if (!recoveredRoutes.includes("core_pool") || recoveredRoutes.includes("user_feedback_pool")) {
    throw new Error(`resolved original event remained trapped in discovery routes: ${JSON.stringify({ routes: recoveredRoutes, scores: recoveredMeta.scores, sourceRole: sourceRoleFor(recoveredOriginalEvent, recoveredOriginalEvent.snapshot), gate: commercialSignalHardGate(recoveredOriginalEvent, recoveredOriginalEvent.snapshot.text, structuredKeyExcerpts(recoveredOriginalEvent, recoveredOriginalEvent.snapshot.text)) })}`);
  }
