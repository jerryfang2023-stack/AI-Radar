...
test("historical quality distinguishes incomplete work and renders all months on desktop and mobile", async () => {
  ...
  try {
    ...
    const entities = JSON.parse(fs.readFileSync("01-SiteV2/site/data/china-funding-entity-health-v1.json", "utf8"));
    await page.evaluate(({ history, entities }) => {
      window.WaveSightOpsConsole = { quality: { chinaFunding: { sources: [] }, chinaFundingHistory: history, chinaFundingEntities: entities } };
      document.querySelector("[data-ops-console]").hidden = false;
    }, { history: { ...history, status: "processed_with_gaps" }, entities });
    ...
    for (const width of [1440, 390]) {
      ...
      assert.match(await panel.innerText(), /国内公司与人物库同步/);
      assert.match(await panel.innerText(), new RegExp(String(entities.accepted_people)));
