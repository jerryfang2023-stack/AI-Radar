...
try {
  ...
  assert.deepEqual(await page.locator('.dc-direction-stats dd').allTextContents(), ['12','179','15']);
  assert.equal(await page.locator('[data-direction]').count(),12);
  await page.locator('[data-direction="apps"]').press('Enter');
  assert.equal(new URL(page.url()).searchParams.get('direction'),'apps');
  assert.ok((await page.locator('#scys-direction-detail').innerText()).includes('21 篇帖子'));
  assert.ok((await page.locator('.dc-direction-fees').innerText()).includes('9,000'));
  assert.ok((await page.locator('.dc-direction-fees').innerText()).includes('竞品价格'));
  const sourceLink = await page.locator('.dc-direction-source a').first().getAttribute('href');
  await page.locator('.dc-direction-source [data-community-open]').first().click();
  assert.equal(await page.getByRole('link',{name:'原始帖子'}).getAttribute('href'),sourceLink);
  await page.getByLabel('关闭',{exact:true}).first().click();
  await page.locator('[data-community-page="2"]').first().click();
  assert.equal(await page.locator('.dc-scys-case').count(),9);
  await page.reload({waitUntil:'networkidle'});
  assert.equal(await page.locator('[data-direction="apps"]').getAttribute('aria-pressed'),'true');
  await page.locator('[data-direction="enterprise"]').click();
  assert.ok((await page.locator('.dc-direction-fees').innerText()).includes('预约报价'));
  assert.ok((await page.locator('.dc-direction-fees').innerText()).includes('不等于正式版成交'));
  ...
  await page.locator('[data-direction="writing"]').click();
  assert.ok((await page.locator('.dc-direction-fees').innerText()).includes('未披露明确收费'));
  assert.equal(await page.locator('.dc-scys-case').count(),3);
