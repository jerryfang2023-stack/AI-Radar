...
try {
  ...
  assert.equal(await page.locator('.dc-scys-case').count(),6);
  ...
  assert.equal(await page.locator('[name="date"], [name="industry"]').count(),0);
  assert.ok((await page.getByLabel('周次',{exact:true}).innerText()).includes('2026-09-07 — 2026-09-13'));
  assert.ok((await page.locator('.dc-weekly-card').allTextContents()).some(x=>x.includes('已处理')));
  await page.getByLabel('周次',{exact:true}).selectOption('2026-08-31');
  await page.waitForFunction(()=>document.querySelector('.dc-empty')?.textContent.includes('本周尚无'));
  await page.getByLabel('周次',{exact:true}).selectOption('2026-09-07');
