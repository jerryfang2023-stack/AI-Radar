...
function validCard(overrides = {}) {
  return {
    ...
    why_now: "两条不同主体的证据同时显示模型成本差异与自动路由需求正在出现。",
    counter_signal: "如果企业持续使用单一模型且不采购独立治理层，该判断被推翻。",
    unknowns: ["企业是否愿意把模型选择交给第三方网关？", "路由延迟是否会抵消成本收益？"],
    validation_action: "访谈使用多模型的工程负责人并核对实际切换流程。",
    minimum_evidence: 2,
    evidence_card_ids: ["SIG-A", "SIG-B"],
    ...overrides,
  };
}

test("Direction Card editorial gate requires judgment, falsifiability, and bounded evidence", () => {
  ...
  assert.ok(directionCardEditorialProblems(validCard({ minimum_evidence: 5 })).includes("minimum_evidence_must_match_ids"));
});

test("Direction Card payload rejects unsupported factual numbers and single-actor evidence", () => {
  const manifest = {
    evidence: [
      { id: "SIG-A", actor: "A", type: "product_service", title: "模型路由发布", source_excerpt: "支持按成本切换模型" },
      { id: "SIG-B", actor: "B", type: "case", title: "企业采用路由", source_excerpt: "企业开始治理模型调用" },
    ],
  };
  assert.deepEqual(directionCandidatePayloadProblems({ candidates: [validCard(), validCard({ id: "DIR-20260725-02", title: "模型预算正在从采购转向运行治理" })] }, manifest), []);
  const problems = directionCandidatePayloadProblems({
    candidates: [
      validCard({ hypothesis: "工程团队需要按任务自动选择模型，并声称可以降低30%的成本，因此愿意为模型网关付费。" }),
...
test("generated and reviewed Direction Cards preserve DeepSeek V4 Pro provenance", () => {
...
test("weekly automation generates candidates without auto-publishing them", () => {
...
test("Direction Card evidence manifest uses accepted source-backed Cards", () => {
  const manifest = buildDirectionEvidenceManifest(root, { asOf: "2026-07-25", limit: 20 });
  assert.equal(manifest.active_date, "2026-07-25");
  assert.equal(manifest.window_days, 30);
  assert.ok(manifest.evidence.length > 5);
  assert.ok(manifest.evidence.every((item) => item.id && item.source_url && item.source_excerpt));
});
