...
2. `hard_gate_scope`
   - Pass when hard gates cover minimum selected/routed/core evidence supply plus evidence integrity only.
...
4. `diagnostics_visible`
   - Pass when captured/selected/routed evidence targets, keyword paths, AI relevance, off-topic titles, importance gaps, concentration, failed sources and fallback remain visible.

5. `semantic_ownership`
   - Pass when Claim/Event, application, and frontstage defects are left to their stage-owned gates instead of duplicated here.
