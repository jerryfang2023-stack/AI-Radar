...
metadata:
  guanlan:
    ...
    recent_learning: "Use the CB Insights AI 100 2026 market hierarchy as the exclusive public market position, keep product form independent, and persist every classification on the source card instead of guessing it in the public builder."
...
6. Normalize the financing round into `round_code` plus a canonical Chinese `round`, preserve `round_original`, build the structured investment thesis and customer research status, and resolve product/founder links by canonical exact match or an accepted same-type mapping in `entity-link-decisions.json`. Persist `analysis.taxonomy_version=TAG-V4.1`, one CB Insights AI 100 2026 market category, its governed subcategory where applicable, one infrastructure application where applicable, one independent primary product form, plus supported use-case, industry, and target-user Facets. Do not mix market position, product form, enabling technology, feature, industry, or future application into one flat classification. Do not publish a card whose source classification is absent; the public builder must not guess from keywords.
