...
metadata:
  guanlan:
    ...
    gates: "unique-event ownership, normalized rounds, current-round investor separation, company-round aggregation, explicit primary product form, captured-source exact quotes, structured thesis and customer status, entity-review queue, DeepSeek provenance, schema, automatic publication, frontstage regression"
    recent_learning: "A first-match keyword classifier confuses enabling technology, future use cases, and the product customers actually buy. Require one explicit primary product form, use reviewed historical decisions where needed, and keep heuristics only for legacy compatibility."
...
6. Normalize the financing round into `round_code` plus a canonical Chinese `round`, preserve `round_original`, build the structured investment thesis and customer research status, and resolve product/founder links by canonical exact match or an accepted same-type mapping in `entity-link-decisions.json`. Require `analysis.product_form_id` to name the one primary product form customers buy or users directly use. Do not classify from a technology mentioned in the product, a feature such as privacy, a target industry, or a future application. Historical corrections belong in `product-form-decisions.json`; the old keyword classifier is fallback compatibility only.
...
5. Confirm every newly generated card has a valid explicit `analysis.product_form_id`, every reviewed legacy decision resolves to an active `product_form` Facet, and the public projection records whether the result came from the card, manual review, or legacy fallback.
