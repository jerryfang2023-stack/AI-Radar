...
Freshness must use the original page's captured publication date before a search provider's inferred date. Prefer a dated URL, structured article metadata, or the visible publication date near the article heading over provider timestamps; a stale article must not become a current-day high-priority title repair merely because search returned it today.
