...
- When a source-quality defect genuinely requires a same-date full rerun, preserve the readable dated source snapshots referenced by already published formal Cards as carry-forward inputs; do not let live-search nondeterminism erase gate-passed same-date evidence.
...
- AI hardware original-source search must preserve query diversity across investment/funding, customer deployment, and product/innovation launch themes. Select at least one commercially oriented query from each theme and apply the current-month recency hint; do not let the first hardware theme consume every daily query slot.
- After preserving the three AI hardware themes, use the remaining configured path slots for high-priority official newsroom or press-release queries. Do not hard-cap the hardware lane below `search-path-query-limit`, and rank `site:` / official-source queries ahead of additional generic variants so same-day reruns do not lose a previously discoverable first-party launch.
- When QC proves that a qualified dated official article is being missed because search ranking returns older pages, add it to the bounded `curated_original_sources` recall list with a publication date and active window. The direct item must still pass normal snapshot, freshness, evidence, dedupe, Card, and editorial gates; never use this list for undated homepages, opinions, or permanent manual promotion.
...
- Funding-process wording such as `help raise`, `ran its own fundraise`, `still coming together`, or `on track rather than closed` is not a completed financing event. Keep it backend-only, and deduplicate confirmed rounds across dates/media by company + amount + round before creating a new Card.
- Funding conflict checks apply to the entire normalized company + amount + round cluster. If one source says pending or unclosed, alternate secondary headlines cannot publish the event as completed until a primary source or named investor resolves the contradiction.
- English source-title localization must use a controlled model prompt or an exact deterministic business-event rule. Generic public machine translation and legacy entries marked `mymemory_title_translation` are not approved title assets and must not satisfy Raw/Card/frontstage/supervision translation coverage.
- Deterministic title localization must cover recurring commercial-event grammar, including `X and Y expand partnership to ...` manufacturing/capacity changes and `X ships N products ... deploys at ...` customer or production deployments. Preserve named companies, products, quantities, customers, and locations; do not require one exact title per article.
- Card fact extraction must derive multiple distinct source-backed facts for high-value partnership, capacity, shipment, and deployment events. A translated title is not a substitute for the news fact, and the same raw excerpt must not fill both `原文要点` and `可见原文片段`.
- Original-page publication evidence outranks search-provider freshness. A dated URL, structured article date, or visible date near the article heading must override a later provider timestamp before current-day recall and title-repair priority are evaluated.
- Product rumor markers such as `爆料`, `传闻`, `泄露`, or `据称` must be recognized anywhere in the title, while source-backed `How X used Y to achieve Z` case titles must remain recallable after deterministic title localization.
