...
- The Reports Center regression test must derive the expected latest issue from source frontmatter so a newly published report cannot leave a stale first-screen card behind.
...
- User-deleted elements must stay deleted unless explicitly requested again: explanatory subtitle under the Industry Reports weekly entry, detail-page hero deck paragraph, `返回周报列表`, `返回情报地图`, `查看行动结论`, visible `日期 / 范围 / 版本` cards, backend version/method/footer text, Trend Candidates, and History blocks.
