# Good Timeline Entry Example

Use this pattern when checking the post-merge First-Line Viewpoints projection in the external Guanlan AI Vault.

```yaml
---
date: 2026-06-12
person: example-builder
source_platform: x
source_url: https://x.com/example/status/123
source_author: Example Builder
source_published_at: 2026-06-12T01:30:00Z
translation_status: translated
formal_tags:
  track: [agent]
  function: [product]
  scenario: [builder-workflow]
---

## Viewpoint

zh_summary: "Polished Chinese summary of the builder's public viewpoint."

original_excerpt: "Short source excerpt or paraphrase boundary, not a full copied thread."

why_it_matters: "What this viewpoint helps operators understand; not a Business Signal fact."
```

Pass criteria:

- The builder has one deduplicated person timeline under `60-知识资产/一线人物/`.
- Original URL, author, and timestamp are present.
- Primary frontstage text is translated or rewritten for Chinese readers.
- The entry is clearly a viewpoint, not evidence for Claims, CanonicalEvents, relations, or trend candidates.
