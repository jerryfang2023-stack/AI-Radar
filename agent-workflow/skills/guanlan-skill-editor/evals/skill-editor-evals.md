...
6. Fail when a project skill is edited but its repo runtime `.agents/skills` mirror is left unsynced. Pass when the external `.skill-store` stays unchanged outside authorized scope; fail when an explicitly authorized compatibility sync is omitted.
...
14. Fail when ordinary domain Skills copy model-specific API parameters, Pro mode, caching, or reasoning settings without owning that API surface and a measured need.
