...
1. `source_near_fields`
   - Pass when map cells are derived from Claim-bound application assertions with resolvable Event and Source refs.
...
5. `evidence_boundary`
   - Pass when First-Line Viewpoints and Community Intelligence are excluded from direct map evidence; a separately captured original source must pass the V4 Claim/Event chain.
...
8. `evidence_modal_presentation`
   - Pass when clicking a hot map cell opens related Event/Claim/Source evidence in a modal or equivalent detail layer.
...
11. `reviewed_direction_cards`
   - Pass when every Direction Card is human-reviewed, resolves to at least two accepted CanonicalEvents with Claim/Source refs and original-source URLs, and states unknowns plus a first validation action.
...
13. `deepseek_v4_pro_editorial_provenance`
   - Pass when Direction Card titles and editorial fields are generated by `deepseek-v4-pro` from a bounded accepted V4 evidence manifest and provenance is persisted.
   - Fail when titles or content come from a local template, a fallback model, uncited external facts, or missing model provenance.

14. `candidate_review_boundary`
   - Pass when model output remains `pending_human_review` until unsupported numbers, hype language, evidence IDs, actor diversity, judgment depth, and counter-signals pass gates plus human review.
   - Fail when a scheduled model call directly overwrites or publishes `opportunity-direction-cards.json`.
