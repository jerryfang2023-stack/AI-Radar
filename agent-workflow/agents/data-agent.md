...
它负责来源、证据和结构化事实生产：V4 主链生成 SourceArtifact、RawDocument、Claim、Entity、CanonicalEvent 与受证据约束的投影；V3 Raw / Pool / Card 只保留为内部兼容和下游应用输入。
...
- 不决定一级栏目和商业化路径。
- 不在 V4 canonical tables 中写机会分析、趋势判断、建议、重要性或其他决策字段。
- 不写最终前台文案。
- 不设计页面信息架构。
- 不实现页面或发布网站。
...
当事实进入 V4 前台投影、行业报告或内部 Card / 关系图 / 趋势兼容链前，Intelligence Engine 必须交接：
...
- V4 Claim / Event 的原始证据或精确 span 不足。
- 下游趋势候选所需的多事件、多来源证据不足。
- AI 商业热力图所需岗位 / 流程映射不足。
...
Experience & Editorial 负责 V4 页面表达、报告阅读体验和 Typography。Intelligence Engine 只交付事实、证据、结构化投影与明确边界，不直接写判断型正文。
