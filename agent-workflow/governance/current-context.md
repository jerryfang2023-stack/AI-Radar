...
- 当前阶段：V2.1 生产开发。
- 当前战略 / 产品基座：`WSD-20260518-GRILLME-STRATEGY-PRODUCT` + `WSD-20260518-RAW-POOL-CARD-RULES`。
- 当前定位：面向商业决策者的 AI 机会判断系统，不是 AI 新闻站、热点导航、工具目录或简单日报。
...
- Pool：20-40 条。
- 今日观察精选变化卡：5-8 条。
- 三段式默认入口：AI HOT 全量 + follow-builders 全量 + 关键词规则补齐；不足或重要卡片缺证时启动外部多路搜索。
- Raw 是原始证据仓库和内容加工入口；Pool 是候选分流层，不替代 Raw 正文。
- 正式 / 前台变化卡、案例卡、趋势报告、今日观察和商业内参事实判断必须满足 Raw 核心证据门槛：`has_full_text=true`、`extraction_quality=high|medium`、`source_level=S|A|B`。
- 观点卡只证明“谁在何时何处说了什么”；观点中的公司动作、客户采用、收入、融资、市场规模等事实主张必须另补 S/A/B 来源。
...
- 旧 `daily-monitor-router` 自动化名。
- 旧 `v2-content-site-daily-update` 自动化本体。
- 旧 `unified-site-sync` 同步闸门。
- 旧 `Structured / Front Signal` 作为正式阶段命名。
- 旧“机会列表 / 评分前台化 / 新闻资讯站”产品口径。
