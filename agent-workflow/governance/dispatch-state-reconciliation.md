...
- 旧 `04-Site/` 只存在于 `10-Archive/v1.0/site/04-Site/`，仅作历史参考。
- 新页面、样式、脚本、内容同步、截图验收和部署任务不得默认写 `04-Site/`。
- 治理文件中保留的 `04-Site` 命令只作为 V1 历史探针或兼容说明；V2 任务必须优先使用 V2 路径和 V2 quality gates。
