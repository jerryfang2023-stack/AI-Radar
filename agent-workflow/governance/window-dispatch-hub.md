...
凡任务涉及首页、栏目页、详情页、今日观察、商业信号、趋势追踪、商业内参、前沿观点、会员页、Admin 或任何 `01-SiteV2/site/*.html` / `01-SiteV2/site/assets/*.css` / `01-SiteV2/site/assets/*.js` 页面体验改动，调度中枢必须把该任务标记为“页面类任务”，并在派发单中加入以下硬性要求。旧 `The Point / Daily Brief / Signals / Opportunities / Trends / Front Signal / Structured Signal / 04-Site` 不作为当前页面范围、当前栏目名或开发入口。
...
页面类任务还必须要求 UI/UE 输出审美标准与桌面证据表。该表既是创作要求，也是交付后的复核依据；必须包含桌面截图、逐项扣分原因、必须重做清单和 Dev 实现偏差清单。当前 V2.1 采用桌面端优先，移动端只有在派发单明确要求专项时才作为硬阻塞；否则记录基础不崩坏 / 无严重横向溢出的非阻塞观察。
...
#### 5.1.2A UI/UE 审美标准与桌面证据表必填字段

派发单必须要求 UI/UE 输出以下评分表。每项 0-20 分，总分 100 分。

质检必须基于截图和具体扣分原因。页面已实现或已有可访问页面时，缺少桌面截图、逐项扣分原因、必须重做清单或 Dev 实现偏差清单的，质检无效。移动端专项只有在派发单明确要求时才必须提供截图。
...
- 设计基准引用：
- 桌面端截图：
- 移动端观察：如派发单明确要求移动端专项，则填写截图路径；否则记录基础观察或“不适用”。
- 逐项扣分原因：
- Dev 实现偏差清单：
- QA 复核建议：
...
- 是否影响其他栏目标题、全站字体层级、Admin 模块节奏或移动端基础可用性。
...
- 今日观察出现行动指令，商业信号把单条新闻写成确定机会，趋势追踪或商业内参中的机会判断把公司名清单包装成机会结论。
...
3. Dev 是否逐条对应实现。
4. QA 是否有桌面端截图；如派发单要求移动端专项，是否有移动端截图。
5. QA 是否按页面规范表做坐标/字号/间距/首屏节奏验收。
6. QA 是否按 Copy 文案规范表做禁用语、判断边界、标题长度和容器适配验收。
7. 是否说明未满足项和阻塞项。
8. 是否包含 UI/UE 审美标准与桌面证据表，且有桌面截图、扣分原因、必须重做清单、Dev 偏差清单。
...
凡任务涉及首页、栏目页、详情页、会员页、Admin、CTA、卡片标题、空状态、错误提示、订阅/登录注册、今日观察、商业信号、趋势追踪、商业内参、前沿观点或任何公开前台可见文案，调度中枢必须把该任务标记为“文案类任务”。
...
- 今日观察输出“下一步应该做、建议立即验证、必须进入验证清单”等行动指令。
...
10. 如涉及页面类任务，检查 UI/UE 审美标准与桌面证据：缺少桌面截图、扣分原因、必须重做清单或 Dev 偏差清单时，退回补齐；低于页面类型通过线、任一单项低于 14、Squint Test 不通过或有审美阻塞项时，退回重做。移动端专项只在派发单明确要求时作为硬阻塞。
