...
它不新增长期 Agent，也不允许用临时 agent 替代长期岗位。它解决的是：派发任务时，除了说明“谁负责”，还必须说明“用什么执行模式”，避免任务提示词越来越长、执行窗口漏读规范、实现和验收混在一起、无效任务质量被误当作新基线。
...
| Pipeline | 多阶段流水线 | 自动化、内容入库、页面开发、部署、迁移、存量资产处理 | 阶段顺序、硬闸门、失败降级、收口 | 不跳过中间质量门 |
...
- V2 页面任务不得继承用户不满意的页面验收作为质量基线；必须重新诊断和评分。
