...
- 手机号、邮箱验证码通过 `VERIFICATION_WEBHOOK_URL` 对接实际发送服务，凭据仅放服务器 `.env`。未配置时生产环境明确返回“验证码通道未配置”，不会在响应中泄露验证码。
- PC 微信登录使用小程序码：PC 创建短时票据，小程序 `pages/account-qr/index` 确认后由 PC 轮询完成登录或绑定。
- 手机、邮箱、微信身份发生冲突时不自动合并；用户必须同时证明当前账户和目标身份，确认后才合并会员、积分和订单，并保留审计记录。
