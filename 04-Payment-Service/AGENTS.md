...
- Mini Program user management under `/api/v1/admin/analytics/membership/users` requires `ANALYTICS_ADMIN_TOKEN`, returns a fixed `MEMBER-ADMIN-V1.0` projection without raw WeChat identities, and exposes only audited entitlement extension and available-point adjustment. Do not add account deletion, identity merge/edit, order mutation, arbitrary expiry replacement, or lifetime-point rewriting to this surface.
