...
### 2026-09-14 第二季注册与账号同步核验

- 修复新注册账号直接使用审核状态、忽略 `communityState` 的问题：审核通过但未登记入群的成员返回 `not_joined`，不显示已入群、不发放社群权益；已入群成员按原规则同步资料、积分和一次性 90 天权限。
- 生产名单有 30 位第二季已入群成员；通过已核验手机号和已有绑定确认 3 个小程序账号，其中 2 位已入群、1 位未登记入群。通过现有 `/member/me` 业务流程刷新这 3 个账号，未创建账号、未按昵称认领、未修改社群名单。
- 两个已入群账号的第二季活动接口均返回 200，未入群账号返回 403；重复同步后资料、钱包和权益不变。其余 28 位已入群成员尚未找到可确认的小程序账号匹配，注册时应授权与社群登记一致的手机号。
- 同步前使用 SQLite 在线备份，备份及逐账号回执仅存服务器私有目录 `/opt/guanlan-backups/season2-account-sync-20260914T123136Z`。仓库不保存个人信息或账号令牌。
- 服务端回归 81 项通过，包含注册时已入群、未入群、已淘汰三种状态，以及重复同步不重复授予积分或权益。客户端版本无需变更。
