...
export function build(inputFile = defaultInput) {
  ...
  const bundledDetails = Object.fromEntries(
    projected.index.cards
      ...
      .map((card) => [card.id, projected.details[card.id]])
  ...
  // The complete detail set is fetched from the live API. Keep only recent
  // records as an offline fallback so the WeChat main package stays below 2 MiB.
  writeModule(path.join(dataDir, "funding-details.js"), bundledDetails, "// Generated by scripts/build-funding-data.mjs. Do not edit.");
  return projected;
}
