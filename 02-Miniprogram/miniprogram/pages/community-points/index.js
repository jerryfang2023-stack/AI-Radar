...
Page({
  ...
  onLoad(options = {}) {
    ...
    if (!preview && this.data.mode !== "rules") this.refresh();
  },
  async refresh() {
    this.setData({ loading: true, error: "" });
    try {
      const result = await communityRequest("points");
      ...
      this.setData({ ...result, ledger, latestPoints: ledger[0]?.points || "—", updatedAt: result.updatedAt.slice(5) || "—" });
    } catch (error) { this.setData({ error: error.message, leaderboard: [], ledger: [], myPoints: 0, myRank: "—", latestPoints: "—" }); }
