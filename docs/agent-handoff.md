...
Use `docs/daily-production-recovery.md` for recovery. The 2026-09-09 accepted core production run is `34310028499`, its publication PR is #833, and private-evidence HEAD is `cd7ba95b746ff2f36a32ec79d9a3af31efce8df9`. PRs #831/#832 repair crore parsing and model amount preservation. Funding recovery reuses the three captured-source Mistral research cards from that run, repairs deterministic amount/history projection, and excludes the valuation-only DeepSeek card without new research calls. Empty GitHub check jobs are not acceptance: run `34310519634` explicitly passed Windows/Linux checks on the core publication commit. Runtime final-closure reports and live receipts, not this handoff, establish deployment completion. Browser control remained unavailable; data checks are not PC/mobile visual acceptance. The primary checkout contains concurrent community Token/member-development edits; do not reset, commit or deploy them as part of daily production.
