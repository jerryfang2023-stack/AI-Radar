...
- Current backend: `OPS-V3.4.1-community-approval` at `https://www.zkdlj.vip/ops/`, shared by the Data Center, Funding Portal, Mini Program, H5 and community membership. The whole console, scripts and operational snapshots are protected by an allowlisted email challenge plus an HttpOnly VPS session; GitHub Pages excludes them. The membership panel reuses that session for Mini Program user operations and native community-member application approval, with no separate login. Integration/local settings and cross-platform Skill sync are documented in `docs/operations-console.md`.
