...
- Current backend: `OPS-V3.5.0-membership-management` at `https://www.zkdlj.vip/ops/`, shared by the Data Center, Funding Portal, Mini Program, H5 and community membership. The whole console, scripts and operational snapshots are protected by an allowlisted email challenge plus an HttpOnly VPS session; GitHub Pages excludes them. Membership & Entitlements has a summary page plus persistent Community Application Review and Mini Program Member Management subpanels in the left navigation; both reuse the console session and load only their own protected data when opened. Integration/local settings and cross-platform Skill sync are documented in `docs/operations-console.md`.
