...
export default function Prototype() {
  ...
  const page = (() => {
    ...
    if (view.kind === "compare") return <CompareView cards={selected.map((id) => cards.find((card) => card.id === id)).filter(Boolean) as FundingCard[]} onBack={back} onRemove={(id) => { setSelected((current) => current.filter((item) => item !== id)); setToast("已取消该公司比较"); }} />;
    ...
    return <TerminalView index={fundingIndex} cards={filteredCards} query={query} onQuery={setQuery} selected={selected} onOpen={openFunding} onSelect={(id) => setSelected((current) => current.includes(id) ? current.filter((item) => item !== id) : current.length < 3 ? [...current, id] : current)} onFilter={() => setFilterOpen(true)} onCompare={() => selected.length >= 2 ? go({ kind: "compare" }) : setToast("请至少选择 2 家公司")} />;
  })();
...
function TerminalView(props: { index: FundingIndex; cards: FundingCard[]; query: string; onQuery: (value: string) => void; selected: string[]; onOpen: (id: string) => void; onSelect: (id: string) => void; onFilter: () => void; onCompare: () => void }) {
  ...
  return <><AppHeader title="融资情报" /><MobileScroll className="app-screen"><main className="screen-content terminal-screen">
    ...
    <section className="funding-metrics"><span><small>融资</small><strong>{props.index.meta.cardCount}</strong></span><span><small>本周</small><strong>{weekCount}</strong></span><span className="funding-date"><small>更新</small><strong>{props.index.meta.latestDate}</strong></span></section>
...
function ProfileView({ nickname, favorites, history, follows, growth, level, membership, onOpen }: { nickname: string; favorites: number; history: number; follows: number; growth: GrowthState; level: ReturnType<typeof levelFor>; membership: ReturnType<typeof membershipFor>; onOpen: (kind: "saved" | "history" | "follows" | "growth" | "membership" | "invite" | "profile-edit") => void }) {
  ...
  const completedTaskCount = tasks.filter((task) => growth.completed.includes(task.id)).length;
  return <><AppHeader title="我的" brand={false} /><MobileScroll className="app-screen"><main className="screen-content profile-screen">
    <button className="identity-row" onClick={() => onOpen("profile-edit")} aria-label="打开个人设置"><img src="/brand/app-icon-light.svg" alt="个人头像" /><span><strong>{nickname}</strong><em>资料完善度 60% · 管理个人信息</em></span><i className="identity-action">设置</i></button>
    <button className="growth-card" onClick={() => onOpen("growth")}><span className="growth-top"><span><em>本周情报成长</em><strong>L{level.level} {level.name}</strong></span><span><strong>{growth.balance}</strong><em>活跃积分</em></span></span><span className="growth-meta"><i>已完成 {completedTaskCount}/{tasks.length} 个成长任务</i><i>距下一级 {Math.max(0, level.next - growth.lifetime)} 分</i></span><span className="progress"><i style={{ width: `${level.progress}%` }} /></span><span className="growth-footer">继续研究，积累积分兑换会员权益</span></button>
    ...
    <SectionTitle title="成长任务" note="查看明细" onClick={() => onOpen("growth")} /><section className="profile-list">{tasks.map((task) => <button key={task.id} onClick={() => task.id === "favorite" ? onOpen("saved") : undefined}><span><strong>{task.title}</strong><em>今日进度 {task.progress}/{task.target}</em></span><i className={growth.completed.includes(task.id) ? "done" : ""}>{growth.completed.includes(task.id) ? "已完成" : `+${task.points} 分`}</i></button>)}</section>
...
function FundingDetailView({ card, watched, compared, onBack, onFavorite, onToggleCompare, onOpenEntity }: { card?: FundingDetail; watched: boolean; compared: boolean; onBack: () => void; onFavorite: () => void; onToggleCompare: () => void; onOpenEntity: (type: EntityType, key: string) => void }) {
  ...
  return <><AppHeader title="融资详情" onBack={onBack} /><MobileScroll className="app-screen"><main className="screen-content detail-screen"><button className="detail-identity entity-jump" onClick={() => onOpenEntity("companies", companyEntityKey(card.company))}><span>{card.initial}</span><div><h2>{card.company}</h2><p>{card.category} · {card.productForm || card.subcategory}</p></div><em>企业档案 ›</em></button><section className="fact-grid"><div><span>本轮金额</span><strong>{card.amount}</strong></div><div><span>融资轮次</span><strong>{card.round}</strong></div><div><span>披露日期</span><strong>{card.date}</strong></div><div><span>累计融资</span><strong>{card.cumulativeAmount}</strong></div></section><section className="detail-actions"><button className={`favorite-action ${watched ? "active" : ""}`} onClick={onFavorite}>{watched ? "已收藏" : "收藏情报"}</button><button className={`compare-action ${compared ? "active" : ""}`} onClick={onToggleCompare}>{compared ? "已加入公司比较" : "加入公司比较"}</button></section><ArticleSection title="公司做什么"><p>{card.companySummary}</p><small>总部：{card.headquarters}</small></ArticleSection>{card.founders.length ? <ArticleSection title="人物库">{card.founders.map((founder) => <button className="simple-row relation-row" key={founder.name} onClick={() => onOpenEntity("people", personEntityKey(founder, card.company))}><span><strong>{founder.name}</strong><small>{founder.role || "创始团队"}</small></span><em>查看人物 ›</em></button>)}</ArticleSection> : null}<ArticleSection title="机构库">{card.investors.length ? card.investors.map((item) => <button className="simple-row relation-row" key={item.name} onClick={() => onOpenEntity("investors", investorEntityKey(item.name))}><span><strong>{item.name}</strong><small>{item.role || "角色未披露"}</small></span><em>查看机构 ›</em></button>) : <p>投资方未披露</p>}</ArticleSection>{card.signals.length ? <ArticleSection title="关键进展">{card.signals.map((item, index) => <p className="numbered" key={item}><b>{index + 1}</b>{publicAnalysisText(item)}</p>)}</ArticleSection> : null}{card.capitalJudgment ? <ArticleSection title="资本为什么下注" kicker="观澜分析"><p>{publicAnalysisText(card.capitalJudgment)}</p></ArticleSection> : null}{card.risks.length ? <ArticleSection title="风险与未知">{card.risks.map((item, index) => <p className="numbered risk" key={item}><b>{index + 1}</b>{publicAnalysisText(item)}</p>)}</ArticleSection> : null}</main></MobileScroll></>;
}
...
function SavedView({ cards, onBack, onOpen, onFavorite }: { cards: FundingCard[]; onBack: () => void; onOpen: (id: string) => void; onFavorite: (id: string) => void }) {
  return <><AppHeader title="我的收藏" onBack={onBack} /><MobileScroll className="app-screen"><main className="screen-content sub-screen"><p className="sub-intro">已收藏 {cards.length} 笔融资</p>{cards.length ? cards.map((card) => <FundingRow key={card.id} card={card} watched selected={false} onOpen={() => onOpen(card.id)} onFavorite={() => onFavorite(card.id)} onSelect={() => undefined} />) : <Empty title="还没有收藏融资" copy="在融资详情页点击收藏，即可建立你的观察列表。" />}</main></MobileScroll></>;
}
...
function CompareView({ cards, onBack, onRemove }: { cards: FundingCard[]; onBack: () => void; onRemove: (id: string) => void }) {
  return <><AppHeader title="公司比较" onBack={onBack} /><MobileScroll className="app-screen"><main className="screen-content sub-screen">{cards.length ? <><section className="compare-grid">{cards.map((card) => <article key={card.id}><div className="compare-card-head"><span>{card.initial}</span><button onClick={() => onRemove(card.id)}>取消比较</button></div><h3>{card.company}</h3><dl><dt>本轮金额</dt><dd>{card.amount}</dd><dt>融资轮次</dt><dd>{card.round}</dd><dt>披露日期</dt><dd>{card.date}</dd><dt>市场分类</dt><dd>{card.category}</dd><dt>总部</dt><dd>{card.headquarters}</dd></dl></article>)}</section>{cards.length < 2 ? <section className="compare-shortfall"><strong>还需选择 1 家公司</strong><p>返回融资情报，在详情页加入另一家公司后即可继续比较。</p><button onClick={onBack}>返回融资情报</button></section> : null}</> : <div className="compare-empty"><Empty title="暂无比较公司" copy="返回融资情报，在详情页选择 2–3 家公司进行比较。" /><button className="secondary-action" onClick={onBack}>返回融资情报</button></div>}</main></MobileScroll></>;
}
