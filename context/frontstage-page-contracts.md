...
| Business Signals column | `BSIG-V2.1.1-channel-neutral-card-supply` | Presents all qualified Raw / Pool business signals that pass raw-to-card cardability as Cards in one importance-sorted view, keeps Enterprise AI / FDE as a secondary lens, adds `aiHardwareSignals` as a separate lens-only module, treats Raw-channel/provider diagnostics as non-blocking when Pool/Card supply is sufficient, and keeps Card eligibility channel-neutral by judging original-source evidence. |
