...
| Application Analytics | `01-SiteV2/site/application-analytics.html` | Internal Application Center dashboard for Mini Program and PC traffic, content, registration, and server-confirmed payment performance | Public navigation; embedded admin secrets; raw phone, OpenID, UnionID, IP address, or client-asserted payment truth | service analytics tests + page syntax + privacy-field regression |
...
`data-center.html`, `trend-radar.html`, `application-analytics.html`, and `opportunity-map.html` share the internal sidebar; it must not contain Guanlan Research. Application Center includes 变化雷达 and the internal-only 运营统计 dashboard. `funding-insights.html`, `intelligence-map.html`, `reports.html`, and weekly/monthly compatibility routes contain redirects only. Opportunity Map remains unlisted and `noindex,nofollow`; FDE is an Event Library theme at `data-center.html?view=events&theme=fde`.
