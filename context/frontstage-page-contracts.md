...
| Funding Insights | `01-SiteV2/site/funding-insights.html` | Independent Application Center entry; verified daily funding events trigger secondary source capture and DeepSeek V4 Pro research into company, product, explicit investors, public customers, comparisons, metrics, capital judgment, funding history, related Direction Card, and links to Industry Dossiers and Relationship Map | Missing or unnamed investors; model-only claims without captured-source exact quotes; search snippets as evidence; automatic mutation of V4 canonical entities or relationships; human-review placeholders; cards that fail the deterministic auto-publication gate | schema + explicit-investor gate + exact-quote evidence gate + automatic-publication test + frontstage regression + desktop/mobile visual smoke |
...
| Opportunity Map | `01-SiteV2/site/opportunity-map.html` | Independent Application Center sidebar entry; source-backed Entry Point Map and Product Pain Map as separate full-width maps; a small set of DeepSeek V4 Pro-drafted and human-reviewed Direction Cards with structural judgment and counter-signal; map cells and Direction Cards open evidence in a modal; `industry-reports-frontstage.json` as the dedicated downstream application projection | Unreviewed model output; unsupported factual numbers or promotional conclusions; Monthly / Weekly report cards; map toggle buttons replacing the two standalone maps; persistent right-side Cell Evidence panel; public V3 desk fetch; auto-generated recommendations or opaque scores; Relation Paths; Signal Candidates / 时间聚集 / Tag 聚合 modules | syntax + DeepSeek candidate gate + human-review boundary + data projection + interaction smoke + desktop/mobile visual smoke + frontstage regression |
...
| Opportunity Map column | `OMAP-V1.1.0-direction-cards` | Owns the dedicated application projection, two source-backed matrices, and human-reviewed Direction Cards with click-to-open source evidence. |
...
| Funding Insights column | `FUNDING-INSIGHT-V1.0-auto-published-research` | Owns funding-event-triggered secondary research, explicit investor disclosure, evidence-bounded application judgments, automatic fail-closed publication, and entity/direction cross-links without mutating canonical facts. |
