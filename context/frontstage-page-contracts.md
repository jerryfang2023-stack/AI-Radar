...
Business Signals must present one unified Card view for the active date. Every qualified Core Pool business signal that can become a Card should be rendered as a frontstage Card, sorted by importance / impact from high to low. The page must not split the public experience into Top10 versus candidate-pool modes, and it must not show sorting reasons as visible copy. The Enterprise AI / FDE lens remains a secondary lens backed by the independent FDE Lens Pool and does not introduce a fourth Card type.

## Current Frontstage Versions

| Scope | Version | Meaning |
|---|---|---|
| Main website | `SITE-V3.4.1` | Current release baseline; keeps Reports Center / Opportunity System and changes Business Signals to unified Core Signal Cards. |
| Business Signals column | `BSIG-V1.2.0-core-signal-cards` | Presents all qualified Core Pool business signals as Cards in one importance-sorted view, keeps the 企业AI化 secondary lens, locks public titles to source titles or registered Chinese translations, and treats Raw-channel/provider diagnostics as non-blocking when Pool/Core/Card supply is sufficient. |
...
| Reports Center column | `IMAP-V2.0.0-report-center-opportunity-system` | Hosts Monthly / Weekly reports, renders source-backed Entry Point Map and Product Pain Map as separate full-width maps from `opportunity_signals`, opens cell evidence in a modal, and keeps relation paths as a supporting exploration section. |
