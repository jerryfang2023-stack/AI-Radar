...
|---|---|---|
| Main website | `SITE-V3.4.4` | Current release baseline; keeps Reports Center / Opportunity System, unified Business Signal Cards, the upgraded Enterprise AI / FDE ingestion boundary, and the separate AI Hardware observation lens. |
| Business Signals column | `BSIG-V2.1.0-ai-hardware-lens` | Presents all qualified Raw / Pool business signals that pass raw-to-card cardability as Cards in one importance-sorted view, keeps Enterprise AI / FDE as a secondary lens, adds `aiHardwareSignals` as a separate lens-only module, and treats Raw-channel/provider diagnostics as non-blocking when Pool/Card supply is sufficient. |
