...
| Application Analytics | `01-SiteV2/site/operations-console.html#analytics` | OPS-only navigation, passwordless read-only aggregates for Mini Program and PC traffic, content, registration, and server-confirmed payment; old application-analytics URL redirects | Data Center/Application Center entry; embedded admin secrets; raw identities or events; unauthenticated admin/write operations; client-asserted payment truth | service analytics tests + dashboard behavior and redirect tests + privacy-field regression |
...
| Dashboard | `01-SiteV2/site/operations-console.html` | Unified data center / financing / Mini Program / H5 / community operations; overview, read-only analytics, source quality, classified versions, independent cross-platform Skill Store, integration inventory and device-local settings | Issue-center/task-chain panels returning; deleting underlying production records; public navigation takeover; source versions presented as verified deployments; exposing identities, admin secrets or unauthenticated writes; inventory claimed as runtime installation | test:ops-unified + test:ops-v2 + test:skill-ops + version gate + desktop/mobile smoke |
...
`data-center.html`, `trend-radar.html`, and `opportunity-map.html` share the internal sidebar; it must not contain Guanlan Research or 运营统计. Application Center includes 变化雷达. 运营统计 is owned by `operations-console.html#analytics`, and `application-analytics.html` redirects there. Its read-only aggregates are accessible without a password; hidden navigation and `noindex,nofollow` are not authentication. `funding-insights.html`, `intelligence-map.html`, `reports.html`, and weekly/monthly compatibility routes contain redirects only. Opportunity Map remains unlisted and `noindex,nofollow`; FDE is an Event Library theme at `data-center.html?view=events&theme=fde`.
