...
| Opportunity Map | `01-SiteV2/site/opportunity-map.html` | Independent Application Center sidebar entry; source-backed Entry Point Map and Product Pain Map as separate full-width maps; a small set of human-reviewed Direction Cards derived from accepted map evidence; map cells and Direction Cards open evidence in a modal; `industry-reports-frontstage.json` as the dedicated downstream application projection | Monthly / Weekly report cards; map toggle buttons replacing the two standalone maps; persistent right-side Cell Evidence panel; public V3 desk fetch; auto-generated recommendations or opaque scores; Relation Paths; Signal Candidates / 时间聚集 / Tag 聚合 modules | syntax + data projection + interaction smoke + desktop/mobile visual smoke + frontstage regression |
