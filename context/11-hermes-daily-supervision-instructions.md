...
| 09:55 | Hermes Three-Lane Early Handoff | Check First-Line Viewpoints RSS after the 08:30 local Codex `builder-observation-daily-sync` run and the 09:17 / 09:47 GitHub fallback windows, and repeat Business Signals / Community Intelligence checks. If same-date data is still missing and no run is active, dispatch the RSS route and write the early-handoff report / Codex handoff artifacts. |
...
- First-Line Viewpoints RSS: after the 08:30 local Codex `builder-observation-daily-sync` attempt and the 09:17 / 09:47 GitHub fallback attempts, if same-date builders data / Obsidian person-date timelines are missing and no run is active, dispatch `.github/workflows/daily-first-line-viewpoints-pr.yml`.
