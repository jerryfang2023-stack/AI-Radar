...
- Current site version: `SITE-V3.4.4`.
- Current Hermes supervision contract: `V3.4.3-daily-problem-watchdog`.
- Current Business Signals column version: `BSIG-V2.1.1-channel-neutral-card-supply`.
...
1. Version preflight: confirm only active files agree on `SITE-V3.4.4`, `BSIG-V2.1.1-channel-neutral-card-supply`, and `EAI-V1.2.0-raw-card-ingestion-boundary`. Treat ledger history as history.
...
3. Data quality: check the lane-specific public contract, not generic volume alone.
   - Business Signals: unified `BSIG-V2.1.1-channel-neutral-card-supply` Cards, separate AI Hardware lens-only items, source-first titles/facts, no Top10/candidate split, no backend-only / low-value items.
...
| 09:45 | Business / FDE Recheck | Judge the Business 08:57 primary and 09:27 health dispatch path. Check `BSIG-V2.1.1-channel-neutral-card-supply` unified Cards, separate AI Hardware lens, and `EAI-V1.2.0` FDE boundary separately. If output is unhealthy and no run is active, write a problem report / inbox item; do not dispatch recovery. |
