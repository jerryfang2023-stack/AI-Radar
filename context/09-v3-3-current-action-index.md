...
use_when:
  ...
  - dispatch SITE-V3.4.4 production work
  - distinguish current actions from historical tasks
priority: current
---

# SITE-V3.4.4 Current Action Index

This file is the current action registry for WaveSight AI SITE-V3.4.4.
...
|---|---|---|
| `current` | Active SITE-V3.4.4 production action. | May be used as a default execution route. |
...
Do not mark an old action as `current` just because it existed before. Only a SITE-V3.4.4-compatible route that serves the current asset system can be `current`.

## Current Actions

Only these actions are `current` for SITE-V3.4.4:
