...
SITE-V4.2 automation is lane-independent for production and site-unified for publication. First-Line Viewpoints and Community Intelligence keep independent data gates and are projected into the V4 Data Center shell. Business Signals continues as an internal V3.3.6.3 compatibility lane after the V4 integrity gate. Trend Radar (`TRADAR-V1.0.0-factual-change-explorer`) rebuilds from accepted V4 frontstage facts and passes its own evidence/count gate before publication. Reports Center (`REPORTS-V1.1.0-lane-independent`) publishes monthly / weekly reports through its own content gate and deterministic renderer; an Opportunity Map failure is a non-blocking warning. Opportunity Map (`OMAP-V2.0.0-v4-evidence`) reads `opportunity-evidence-v2.json`, built from accepted V4 events, Claims, sources, entities, and facets. Weekly production asks DeepSeek V4 Pro to write evidence-bounded Direction Card candidates, keeps those candidates pending until human review, and rebuilds public Direction Cards only when all Event, Claim, and Source references resolve. No public page reads the V3 desk JSON. Hermes is limited to a 10:20 control-plane liveness check; routine production supervision and Codex handoff belong to Daily Closure.
...
| Trend Radar / Reports Center / Opportunity Map | GitHub Actions + Trend Radar updater + monthly/weekly report page-generator Skills + Opportunity Map updater | Trend Radar follows accepted V4 materialization; report content passes acceptance before deterministic HTML; map evidence follows the compatibility Card chain and is projected independently | Trend Radar evidence/count/coverage gate + periodic content gate + deterministic report renderer + compatibility Card and opportunity-map gates | included in Data Center, periodic-report, or Business PRs according to the owning application |
...
Weekly and monthly production is owned by `.github/workflows/periodic-reports-pr.yml` under `REPORTS-V1.1.0-lane-independent`. DeepSeek Pro writes source-ID-cited report Markdown; on weekly runs, a separate `deepseek-v4-pro` call writes Direction Card titles and content from accepted V4 Event/Claim/Source evidence into `opportunity-direction-card-candidates.json`. Candidate gates reject unknown evidence, unsupported factual numbers, generic or promotional titles, insufficient actor diversity, and missing counter-signals. Opportunity projection or candidate failure is recorded as a warning and does not block report content generation, acceptance, rendering, or publication. Candidates never overwrite the reviewed public configuration automatically. `assert-periodic-report-content.mjs` accepts or rejects report content; `render-periodic-report-pages.mjs` is the sole HTML, navigation, and report-version writer; and the monthly/weekly report page-generator Skills define page structure and validation. The workflow opens and merges a PR; GitHub Pages deploys from `main`. Local periodic tasks are disabled by default to avoid duplicate production and may be installed only as an explicit recovery lane.
