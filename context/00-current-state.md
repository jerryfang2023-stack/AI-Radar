...
- The current backend is OPS-V3.4.1-community-approval at `https://www.zkdlj.vip/ops/`: one authenticated operations entry for the Data Center, Funding Portal, Mini Program, financing H5 and community membership. The login uses an allowlisted email challenge and an HttpOnly, SameSite=Strict VPS session; the same session now protects native community-member application review and audited approval actions. GitHub Pages excludes the console and its operational assets.
