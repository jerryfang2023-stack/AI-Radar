...
- The current backend is OPS-V3.3.0-passwordless-admin: one operations entry for the Data Center, Funding Portal, Mini Program, financing H5 and community membership. Mini Program user administration uses an allowlisted email code and a short-lived browser session instead of a terminal-sourced token.
