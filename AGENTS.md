...
- Current dashboard source: `01-SiteV2/site/operations-console.html`, `OPS-V3.4.1-community-approval`; deploy only to the authenticated VPS route `https://www.zkdlj.vip/ops/`, never in the GitHub Pages artifact. The allowlisted administrator email challenge establishes an HttpOnly, SameSite=Strict VPS session for the whole console; the membership panel reuses it for Mini Program user management and audited community-member approval, with no separate login. Issue/task UI panels are retired; records remain. System Settings owns integration/freshness information and device-local preferences.
