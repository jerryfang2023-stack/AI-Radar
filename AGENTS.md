...
- Current dashboard: `01-SiteV2/site/operations-console.html`, `OPS-V3.3.1-concise-membership`; unified operations for Data Center, Funding Portal, Mini Program, H5 and community. Issue/task UI panels are retired; records remain. Membership aggregates stay passwordless and identity-free; the UI presents metrics without repeated explanatory copy. Mini Program account details and audited adjustments use an allowlisted admin email challenge plus a short-lived in-memory browser session. System Settings owns integration/freshness information and device-local preferences.
