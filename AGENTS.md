...
- Current dashboard source: `01-SiteV2/site/operations-console.html`, `OPS-V3.5.0-membership-management`; deploy only to the authenticated VPS route `https://www.zkdlj.vip/ops/`, never in the GitHub Pages artifact. The allowlisted administrator email challenge establishes an HttpOnly, SameSite=Strict VPS session for the whole console. Membership & Entitlements keeps aggregate overview separate from its persistent Community Application Review and Mini Program Member Management subpanels; both reuse the console session and load protected data only when opened. Issue/task UI panels are retired; records remain. System Settings owns integration/freshness information and device-local preferences.
