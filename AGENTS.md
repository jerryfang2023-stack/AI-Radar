...
V1 历史商业雷达文件已从当前仓库物理移除；只有用户明确要求追溯时，才允许通过 Git 历史查找，不得把它当作 V2.1 每日生产漏斗或当前验收规则：
...
V1 旧脚本已从当前仓库移除，不作为 V2.1 当前生产线默认验证。
...
从 2026-05-18 起，本项目在 V2-only 基础上进入 V2.1 生产开发口径：

- V1.0 网站不再更新，V1 旧站代码与旧文章归档已从当前仓库移除。
- 旧自动化任务 `ai-the-point`、`ai-2`、`ai-3` 已删除，不再作为后续任务的影响判断对象。
- 后续调度不再要求说明“是否影响旧自动化 / V1 网站 / 旧内容日更”。
- 不再围绕旧 V1 链路做兼容性优先判断；默认目标是推进 V2.1 正式生产线。
- V2.1 以 `WSD-20260518-GRILLME-STRATEGY-PRODUCT` 和 `WSD-20260518-RAW-POOL-CARD-RULES` 为当前战略、产品、内容、证据和自动化基座。
- V2.1 不再兼容旧“新闻资讯 / 机会列表 / 评分前台化 / daily-monitor-router / v2-content-site-daily-update / unified-site-sync”口径。
- V2.1 核心内容生产原则是 Raw-first / Evidence-first：AI HOT、follow-builders、社区和聚合源只能作发现入口，前台事实必须回源。
...
- 自动化、同步脚本、内容路径和数据 schema 的后续改动，只按 V2.1 新生产线验收，不再按 V1 影响口径阻塞。
