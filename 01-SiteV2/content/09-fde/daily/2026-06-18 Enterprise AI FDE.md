...
## 1. 受治理的 AI Agent：如何自信地部署和扩展

- card_id: `POOL-2026-06-18-P-060`
...
- evidence_boundary: 已确认：Boomi 讨论受治理 AI Agent 的部署与规模化，重点是企业在可靠性、安全、治理和集成要求下如何把 Agent 从试点推进到生产环境。；未确认：内部投入、ROI 与长期运行效果。

## 2. Claude 优先的医疗 FDE Pod 实际做什么

- card_id: `POOL-2026-06-18-P-055`
- subject: Genzeon Platforms
...
- evidence_boundary: 已确认：Genzeon Platforms 的 field note 描述 Claude 优先的医疗 FDE pod：由 Domain Operator 和 Forward Deployed Engineer 组成 2 人客户嵌入团队，把 agentic AI 交付到受监管支付方工作流。；未确认：内部投入、ROI 与长期运行效果。

## 3. Ontora：访谈每位员工并将上下文交给 AI 工具的 AI Agent

- card_id: `SIG-20260618-A14`
- subject: Ontora
...
- evidence_boundary: 已确认：Y Combinator 公司 Ontora 提供 AI Agent，用于访谈每位员工并把组织上下文交给企业 AI 工具，核心信号是企业内部知识采集和上下文传递流程的产品化。；未确认：内部投入、ROI 与长期运行效果。

## 4. 发布 Agentic Resource Discovery 规范

- card_id: `SIG-20260618-A11`
- subject: Googleblog
...
- scenario: 软件研发
- workflow: 代码、审查与交付流程
- evidence_boundary: 已确认：Google Developers Blog 原文标题宣布 Agentic Resource Discovery 规范。；未确认：内部投入、ROI 与长期运行效果。

## 5. 应用 AI 案例研究和真实成功故事

- card_id: `SIG-20260618-A05`
- subject: GoGloby
...
- scenario: 客户服务与销售
- workflow: 客服响应、线索跟进与知识检索
- evidence_boundary: 已确认：GoGloby 汇总应用 AI 在客户运营、销售、内容和工作流中的案例，用于观察 AI 是否已经进入真实业务流程和可衡量成效。；未确认：内部投入、ROI 与长期运行效果。
