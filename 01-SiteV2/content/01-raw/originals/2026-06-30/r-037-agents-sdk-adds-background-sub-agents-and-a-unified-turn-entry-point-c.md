...
original_url: "https://developers.cloudflare.com/changelog/post/2026-06-26-agents-sdk-v0170/"
canonical_url: "https://developers.cloudflare.com/changelog/post/2026-06-26-agents-sdk-v0170"
...
author: ""
published_at: ""
collected_at: 2026-06-30T02:26:24.695Z
language: mixed
full_text_hash: d673be4d078f12ef
markdown_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-06-30/r-037-agents-sdk-adds-background-sub-agents-and-a-unified-turn-entry-point-c.md"
json_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-06-30/r-037-agents-sdk-adds-background-sub-agents-and-a-unified-turn-entry-point-c.json"
html_snapshot_path: ""
screenshot_path: ""
...
extractor_diagnostics: {"readability_score":88,"text_length":8193,"paragraph_count":68,"sentence_count":55,"boilerplate_hits":3,"symbol_ratio":0.0052,"replacement_count":0,"replacement_ratio":0,"control_count":0,"control_ratio":0,"binary_marker_count":0,"mojibake_marker_count":0,"binary_contaminated":false,"mojibake_contaminated":false,"method":"main"}
has_full_text: true
content_length: 8193
fetch_error: ""
...
url_hash: 87d069a7b19bbca8
content_hash: d673be4d078f12ef
semantic_hash: d1eed728de85f9d5
duplicate_of: ""
first_seen_at: "2026-06-30T02:26:24.695Z"
last_seen_at: 2026-06-30T02:26:24.695Z
...
business_elements: {"companies":["keyword search","Anysearch"],"products":["Agents","agents","agent","Agent","MCP"],"people":[],"industries":["医疗","开发者工具","企业服务"],"roles":["CIO / IT 负责人"],"workflows":[],"business_actions":["发布 / 推出","部署 / 上线"],"affected_departments":["IT / 安全","销售 / 客服"],"numbers":["26","2026","60","1000","24","0.6","30"],"quotes":["onImportDone","completed","interrupted","onImportDone","completed"]}
...
JavaScript
TypeScript
JavaScript class OrdersAgent extends Think {
...
TypeScript class OrdersAgent extends Think {
async startImport ( input ) {
// Fire-and-forget, or wire a durable completion callback
// (by method name, like schedule()):
await this . runAgentTool ( ImportAgent , {
...
JavaScript
TypeScript
JavaScript // Inside the child sub-agent:
await this . reportProgress ( {
fraction : 0.6 ,
phase : "deploying" ,
message : "Generating menu page…" ,
} ) ;
TypeScript // Inside the child sub-agent:
await this . reportProgress ( {
fraction : 0.6 ,
phase : "deploying" ,
message : "Generating menu page…" ,
...
JavaScript
TypeScript
JavaScript await this . runTurn ( { mode : "wait" , messages } ) ; // saveMessages / continueLastTurn
await this . runTurn ( { mode : "submit" , messages } ) ; // durable submitMessages
await this . runTurn ( { mode : "stream" , messages } ) ; // chat()
TypeScript await this . runTurn ( { mode : "wait" , messages } ) ; // saveMessages / continueLastTurn
await this . runTurn ( { mode : "submit" , messages } ) ; // durable submitMessages
await this . runTurn ( { mode : "stream" , messages } ) ; // chat()
...
- fetch_status: fetched-readable-text-main
- extraction_quality: high
- diagnostics: {"readability_score":88,"text_length":8193,"paragraph_count":68,"sentence_count":55,"boilerplate_hits":3,"symbol_ratio":0.0052,"replacement_count":0,"replacement_ratio":0,"control_count":0,"control_ratio":0,"binary_marker_count":0,"mojibake_marker_count":0,"binary_contaminated":false,"mojibake_contaminated":false,"method":"main"}
...
1. **product_update**｜supports=signal_card_candidate, relationship_graph_input, business_change｜importance=high｜confidence=high
   New updates and improvements at Cloudflare. Subscribe to RSS View RSS feeds ← Back to all posts ## Agents SDK adds background sub-agents and a unified turn entry point ... Jun 26, 2026 Agents Workers The latest release of the Agents SDK ↗ makes it easier to run long work in the background, drive tur... / query=AI coding agent API SDK release enterprise product update 2026 / intent=find_original_source / path=official_original
...
3. **product_update**｜supports=signal_card_candidate, relationship_graph_input, business_change｜importance=high｜confidence=high
   Subscribe to RSS View RSS feeds ← Back to all posts Agents SDK adds background sub-agents and a unified turn entry point Jun 26, 2026 Agents Workers The latest release of the Agents SDK ↗ makes it easier to run long work in the background, drive turns through one entry point, and keep chat agents working through deploys, evictions, and reconnects.
...
5. **company_action**｜supports=signal_card_candidate, relationship_graph_input, business_change, case, trend_candidate_context｜importance=high｜confidence=high
   Background sub-agents with progress and milestones runAgentTool can now dispatch a sub-agent without blocking the calling turn.
...
6. **company_action**｜supports=signal_card_candidate, relationship_graph_input, business_change, case, trend_candidate_context｜importance=high｜confidence=high
   A detached run returns a handle immediately and is owned by a durable, eviction-surviving backbone instead of being abandoned when the dispatching turn ends.
...
- numbers: 26, 2026, 60, 1000, 24, 0.6, 30
- quotes: onImportDone / completed / interrupted / onImportDone / completed

## evidence_seed

- company_actions: New updates and improvements at Cloudflare. Subscribe to RSS View RSS feeds ← Back to all posts ## Agents SDK adds background sub-agents and a unified turn entry point ... Jun 26, 2026 Agents Workers The latest release of the Agents SDK ↗ makes it easier to run long work in the background, drive tur... / query=AI coding agent API SDK release enterprise product update 2026 / intent=find_original_source / path=official_original / Changelog New updates and improvements at Cloudflare. / Subscribe to RSS View RSS feeds ← Back to all posts Agents SDK adds background sub-agents and a unified turn entry point Jun 26, 2026 Agents Workers The latest release of the Agents SDK ↗ makes it easier to run long work in the background, drive turns through one entry point, and keep chat agents working through deploys, evictions, and reconnects.
- case_details: 暂无公开信息
