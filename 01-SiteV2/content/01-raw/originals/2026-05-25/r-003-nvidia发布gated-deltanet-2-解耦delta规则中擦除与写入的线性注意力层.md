...
original_url: "https://www.marktechpost.com/2026/05/24/nvidia-ai-releases-gated-deltanet-2-a-linear-attention-layer-that-decouples-erase-and-write-in-the-delta-rule"
canonical_url: "https://marktechpost.com/2026/05/24/nvidia-ai-releases-gated-deltanet-2-a-linear-attention-layer-that-decouples-erase-and-write-in-the-delta-rule"
...
search_path_label: ""
author: ""
published_at: "2026-05-24T07:42:09.000Z"
collected_at: 2026-05-25T06:12:20.667Z
language: mixed
full_text_hash: 4801893dea751edc
markdown_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-05-25/r-003-nvidia发布gated-deltanet-2-解耦delta规则中擦除与写入的线性注意力层.md"
json_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-05-25/r-003-nvidia发布gated-deltanet-2-解耦delta规则中擦除与写入的线性注意力层.json"
html_snapshot_path: ""
screenshot_path: ""
...
extractor_diagnostics: {"readability_score":91,"text_length":11601,"paragraph_count":106,"sentence_count":110,"boilerplate_hits":2,"symbol_ratio":0.0084,"method":"article"}
has_full_text: true
content_length: 11601
fetch_error: ""
...
discovery_record: {"discovery_title":"NVIDIA发布Gated DeltaNet-2：解耦Delta规则中擦除与写入的线性注意力层","discovery_summary":"NVIDIA推出了改进的线性注意力模型Gated DeltaNet-2。其核心创新在于将Delta规则中控制记忆擦除与写入的单一门控机制，解耦为两个独立的通道级门控。该模型参数规模为1.3B，在100B个FineWeb-Edu令牌数据上训练。实验表明，它在语言建模、常识推理和长上下文检索等任务中全面超越了Mamba-2、Gated DeltaNet、KDA及Mamba-3等现有模型，尤其在RULER S-NIAH和多关键针检索任务中取得了最显著的性能提升。","source_name":"MarkTechPost（RSS）","origin_url":"https://www.marktechpost.com/2026/05/24/nvidia-ai-releases-gated-deltanet-2-a-linear-attention-layer-that-decouples-erase-and-write-in-the-delta-rule","discovered_at":"2026-05-25T06:01:03.456Z","rank_on_page":163,"discovery_status":"discovered"}
source_role: resolved_original_source
origin_fetch_status: "success"
paywall_status: none
block_status: none
duplicate_status: unique
url_hash: 5d57adbe8d961bc0
content_hash: 4801893dea751edc
semantic_hash: 0fdbb990d34b15e4
duplicate_of: ""
first_seen_at: "2026-05-24T07:42:09.000Z"
last_seen_at: 2026-05-25T06:12:20.667Z
...
business_elements: {"companies":["MarkTechPost（RSS）","GitHub","Nvidia"],"products":[],"people":[],"industries":["开发者工具"],"roles":["CIO / IT 负责人","开发者 / 工程团队"],"workflows":[],"business_actions":["发布 / 推出","合作 / 联盟"],"affected_departments":["IT / 安全"],"numbers":["2","1.3B","100B","3","0","1","64","262"],"quotes":[]}
...
title_zh: "NVIDIA发布Gated DeltaNet-2：解耦Delta规则中擦除与写入的线性注意力层"
...
Gated DeltaNet-2 separates the two decisions through Gated Delta Rule-2. It introduces a channel-wise erase gate b t ∈ [0,1] d k on the key axis. It also introduces a channel-wise write gate w t ∈ [0,1] d v on the value axis. Both gates are produced by sigmoid projections of the token representation. The update applies decay before the active edit.
Written compactly, the recurrence is:
S t = (I − k t (b t ⊙ k t ) ⊤ ) D t S t−1 + k t (w t ⊙ v t ) ⊤
Here D t = Diag(α t ) is the channel-wise decay carried over from KDA. The left factor of the erase matrix stays k t , preserving the delta-rule write direction. The right factor becomes b t ⊙ k t , making the read direction channel-selective. The write term k t z t ⊤ uses z t = w t ⊙ v t , making the value update channel-selective.
When both gates collapse to the same scalar β t , the update recovers KDA exactly. When the decay α t also collapses to a scalar, it recovers Gated DeltaNet. Both prior models are preserved as tied subspaces of the new update.
...
Gated DeltaNet-2 &middot; Quickstart
01 / 08
NVIDIA &middot; 2026
Gated DeltaNet-2
...
The Problem
Prior delta-rule models (Gated DeltaNet, KDA) tie erasing old content and writing new content to one scalar gate &beta;_t .
The Fix
...
Step 02 &middot; The Update Rule
The Gated Delta Rule-2
With erase gate b_t &isin; [0,1]^{d_k} , write gate w_t &isin; [0,1]^{d_v} , and channel-wise decay D_t = Diag(&alpha;_t) , the recurrent state evolves as:
S_t = (I &minus; k_t (b_t &odot; k_t) ⊤ ) D_t S_{t&minus;1} + k_t (w_t &odot; v_t) ⊤
...
Repo layout
lit_gpt/ model code &middot; scripts/ launchers &middot; pretrain.py training entry &middot; data.py , cache.py data & KV cache &middot; paper/ arXiv PDF
Step 04 &middot; Launch Training
Run pretrain.py
...
Optimizer
AdamW &middot; peak LR 4e-4 &middot; weight decay 0.1 &middot; gradient clip 1.0 &middot; cosine schedule &middot; 1B -token warmup.
Batch & Sequence
Global batch 0.5M tokens &middot; sequence length 4K &middot; hybrid models use a 2K sliding-window attention size.
Model Shape
16 heads &middot; d_k = d_v = 128 &middot; per-layer recurrent state 262,144 floats, matched against Mamba-2/3.
Hybrid Block
Repeated cell: Gated DeltaNet-2 &rarr; MLP &rarr; SWA &rarr; MLP. The recurrent mixer compresses long histories; SWA handles local interactions.
Step 06 &middot; Results
...
Setting &middot; Metric
KDA
Mamba-3 MIMO
GDN-2
Recurrent avg. (LMB + reasoning)
52.28
52.39
53.11
Hybrid avg. (LMB + reasoning)
52.68
52.72
53.97
S-NIAH-3 @2K (recurrent)
63.2
72.4
89.8
MK-NIAH-1 @4K (recurrent)
28.0
18.0
37.8
Real-world recall, recurrent avg.
28.67
28.35
29.88
Real-world recall, hybrid avg.
40.14
40.11
42.28
Step 07 &middot; Resources
Paper, code, and citation
Everything you need to read, run, and cite Gated DeltaNet-2 in one place.
GitHub &middot; NVlabs/GatedDeltaNet-2
&rarr;
Paper PDF (in repo)
&rarr;
License (NVIDIA SCL-NC)
&rarr;
Predecessor &middot; Gated DeltaNet
&rarr;
...
&larr; Prev
Next &rarr;
MARKTECHPOST &middot; The hub for AI research, dev tools, and model launches
Key Takeaways
Gated DeltaNet-2 splits the scalar β t into a channel-wise erase gate b t (key axis) and a channel-wise write gate w t (value axis).
...
- fetch_status: fetched-readable-text-article
- extraction_quality: high
- diagnostics: {"readability_score":91,"text_length":11601,"paragraph_count":106,"sentence_count":110,"boilerplate_hits":2,"symbol_ratio":0.0084,"method":"article"}
...
2. **product_update**｜supports=daily_observation, heatmap, change｜importance=high｜confidence=high
   Artificial Intelligence AI Infrastructure Tech News AI Paper Summary Applications Technology Editors Pick Language Model Large Language Model Machine Learning New Releases Open Source Physical AI Software Engineering Staff Linear attention replaces the unbounded KV cache of softmax attention with a fixed-size recurrent state.

3. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   This cuts sequence mixing to linear time and decoding to constant memory.

4. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   The hard part is not what to forget.

5. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   It is how to edit a compressed memory without scrambling existing associations.

6. **product_update**｜supports=daily_observation, heatmap, change｜importance=high｜confidence=high
   NVIDIA has released Gated DeltaNet-2 , a linear attention layer that targets that bottleneck.
...
- numbers: 2, 1.3B, 100B, 3, 0, 1, 64, 262
- quotes: 暂无公开信息

## evidence_seed

- company_actions: NVIDIA推出了改进的线性注意力模型Gated DeltaNet-2。其核心创新在于将Delta规则中控制记忆擦除与写入的单一门控机制，解耦为两个独立的通道级门控。该模型参数规模为1.3B，在100B个FineWeb-Edu令牌数据上训练。实验表明，它在语言建模、常识推理和长上下文检索等任务中全面超越了Mamba-2、Gated DeltaNet、KDA及Mamba-3等现有模型，尤其在RULER S-NIAH和多关键针检索任务中取得了最显著的性能提升。 / Artificial Intelligence AI Infrastructure Tech News AI Paper Summary Applications Technology Editors Pick Language Model Large Language Model Machine Learning New Releases Open Source Physical AI Software Engineering Staff Linear attention replaces the unbounded KV cache of softmax attention with a fixed-size recurrent state. / This cuts sequence mixing to linear time and decoding to constant memory.
- case_details: 暂无公开信息
