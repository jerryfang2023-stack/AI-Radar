...
raw_id: R-019
title: "OpenAI Sora API in 2026: How to Use the Official Videos API | LaoZhang AI Blog"
original_url: "https://blog.laozhang.ai/en/posts/openai-sora-api"
canonical_url: "https://blog.laozhang.ai/en/posts/openai-sora-api"
...
author: ""
published_at: ""
collected_at: 2026-06-25T03:26:00.378Z
language: mixed
full_text_hash: 2523e8a4c04dfba8
markdown_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-06-25/r-019-openai-sora-api-in-2026-how-to-use-the-official-videos-api-laozhang-ai.md"
json_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-06-25/r-019-openai-sora-api-in-2026-how-to-use-the-official-videos-api-laozhang-ai.json"
html_snapshot_path: ""
screenshot_path: ""
...
extractor_diagnostics: {"readability_score":97,"text_length":14697,"paragraph_count":121,"sentence_count":115,"boilerplate_hits":0,"symbol_ratio":0.0035,"replacement_count":0,"replacement_ratio":0,"control_count":0,"control_ratio":0,"binary_marker_count":0,"mojibake_marker_count":0,"binary_contaminated":false,"mojibake_contaminated":false,"method":"main"}
has_full_text: true
...
url_hash: b0bde2c8d9848eef
content_hash: 2523e8a4c04dfba8
semantic_hash: ed834febf61ab2a9
duplicate_of: ""
first_seen_at: "2026-06-25T03:26:00.378Z"
last_seen_at: 2026-06-25T03:26:00.378Z
...
business_elements: {"companies":["keyword search","Tavily","OpenAI"],"products":["ChatGPT","Gemini","Claude"],"people":[],"industries":["开发者工具","企业服务"],"roles":["CIO / IT 负责人","开发者 / 工程团队"],"workflows":["合同审阅 / 法律研究","计费 / 预算管理","部署 / 集成交付"],"business_actions":["定价 / 计费变化"],"affected_departments":["IT / 安全","财务 / 预算","销售 / 客服"],"numbers":["2026","2","1","0.180","28","1920x","1080","1080x"],"quotes":["OpenAI Sora API"," language. / score=0.180 / query=AI platform case study production rollout customer 2026 / intent=find_original_source / path=official_original\nIf by ","OpenAI Sora API","the Sora app."," ;\nimport { writeFile } from "]}
...
# OpenAI Sora API in 2026: How to Use the Official Videos API | LaoZhang AI Blog
...
TL;DR
Yes, official developer access exists today. The surface you want is /v1/videos , not the consumer Sora editor.
Use sora-2 first when you are iterating on prompt, framing, or motion at lower cost.
Use sora-2-pro when you need higher-fidelity output, and especially when you need 1920x1080 or 1080x1920 .
Treat the workflow as asynchronous by default. Create the job, poll GET /videos/{video_id} or use webhooks, then download the asset from GET /videos/{video_id}/content .
OpenAI's docs are partially out of sync right now. The current guide and March 2026 changelog document 16/20-second support, video edits, extensions, reusable characters, Batch, and 1080p sora-2-pro , while some older reference snippets still show shorter durations and lower maximum sizes.
...
model : "sora-2" ,
prompt :
"Wide tracking shot of a yellow taxi crossing a rain-soaked city street at blue hour, neon reflections, natural ambient audio." ,
size : "1280x720" ,
seconds : "8" ,
} ) ;
if ( video . status !== "completed" ) {
throw new Error ( ` Video failed with status ${ video . status } ` ) ;
const content = await openai . videos . downloadContent ( video . id ) ;
const buffer = Buffer . from ( await content . arrayBuffer ( ) ) ;
await writeFile ( "video.mp4" , buffer ) ;
// In production, copy the file into your own object storage immediately.
And here is the same lifecycle in raw HTTP form:
bash
curl -X POST "https://api.openai.com/v1/videos" \
-H "Authorization: Bearer $OPENAI_API_KEY " \
-F model = "sora-2" \
-F prompt = "Wide tracking shot of a yellow taxi crossing a rain-soaked city street at blue hour, neon reflections, natural ambient audio." \
-F size = "1280x720" \
-F seconds = "8"
...
If you are exploring prompts, start with sora-2 .
If you are shipping final assets and visual polish matters, move to sora-2-pro .
If you are rendering a shot list offline, Batch changes the economics enough that it is worth serious consideration.
...
the main video guide says both sora-2 and sora-2-pro support 16 - and 20 -second generations,
the March 2026 changelog explicitly says the Sora API expanded to longer generations up to 20 seconds and 1080p sora-2-pro ,
but older create-reference snippets still show the older 4 , 8 , 12 duration set and a smaller size matrix.
...
Batch supports POST /v1/videos only.
Batch requests must be JSON, not multipart.
...
# OpenAI Sora API # Sora 2 # OpenAI Videos API # Video Generation API # OpenAI API
Share :
laozhang.ai
...
TG : @laozhang_cn | @laozhang_cn | Get $0.1
...
- fetch_status: fetched-readable-text-main
- extraction_quality: high
- diagnostics: {"readability_score":97,"text_length":14697,"paragraph_count":121,"sentence_count":115,"boilerplate_hits":0,"symbol_ratio":0.0035,"replacement_count":0,"replacement_ratio":0,"control_count":0,"control_ratio":0,"binary_marker_count":0,"mojibake_marker_count":0,"binary_contaminated":false,"mojibake_contaminated":false,"method":"main"}
...
1. **quote**｜supports=daily_observation, heatmap, viewpoint｜importance=high｜confidence=high
   # OpenAI Sora API in 2026: How to Use the Official Videos API. Yes, OpenAI has an official developer video API today, but the cleanest way to think about it is as the Videos API powered by Sora 2 and Sora 2 Pro. This guide separates the consumer Sora app from the developer surface, shows the fastest working integration path, and explains current pricing, advanced endpoints, and the docs mismatch developers should know about. If by "OpenAI Sora API" you mean official programmatic video generation on the OpenAI platf

2. **quote**｜supports=daily_observation, heatmap, viewpoint｜importance=high｜confidence=high
   If by "OpenAI Sora API" you mean official programmatic video generation on the OpenAI platform, the answer is yes : OpenAI's current developer surface is the Videos API , centered on POST /v1/videos and the sora-2 / sora-2-pro models.

3. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   The part that confuses people is that OpenAI also has a consumer Sora app/editor and ChatGPT-based Sora access , and those surfaces do not map cleanly to the same "API" language.

4. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   That is why the search results still look contradictory in March 2026.

5. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   This guide is written for the developer question, not the app question.

6. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   It focuses on the official OpenAI developer contract that exists today, the fastest working create -> wait -> download flow, the pricing and policy constraints that matter before you build, and the March 2026 changes that made older Sora API summaries incomplete.

## business_elements

- companies: keyword search, Tavily, OpenAI
...
- company_actions: The part that confuses people is that OpenAI also has a consumer Sora app/editor and ChatGPT-based Sora access , and those surfaces do not map cleanly to the same "API" language. / That is why the search results still look contradictory in March 2026. / This guide is written for the developer question, not the app question.
- case_details: 暂无公开信息
...
# OpenAI Sora API in 2026: How to Use the Official Videos API. Yes, OpenAI has an official developer video API today, but the cleanest way to think about it is as the Videos API powered by Sora 2 and Sora 2 Pro. This guide separates the consumer Sora app from the developer surface, shows the fastest working integration path, and explains current pricing, advanced endpoints, and the docs mismatch developers should know about. If by "OpenAI Sora API" you mean official programmatic video generation on the OpenAI platform, the answer is **yes**: OpenAI's current developer surface is the `Videos API`, centered on `POST /v1/videos` and the `sora-2` / `sora-2-pro` models. The part that confuses people is that OpenAI also has a **consumer Sora app/editor** and **ChatGPT-based Sora access**, and those surfaces do not map cleanly to the same "API" language. / score=0.180 / query=AI platform case study production rollout customer 2026 / intent=find_original_source / path=official_original
