...
original_url: "https://developers.googleblog.com/build-agentic-full-stack-apps-with-genkit"
canonical_url: "https://developers.googleblog.com/build-agentic-full-stack-apps-with-genkit"
...
search_path_label: ""
author: ""
published_at: "2026-07-01T16:17:55.480Z"
collected_at: 2026-07-02T02:28:35.175Z
language: mixed
full_text_hash: 5fe82e9cba6c76ab
markdown_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-07-02/r-010-用-genkit-构建智能体全栈应用.md"
json_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-07-02/r-010-用-genkit-构建智能体全栈应用.json"
html_snapshot_path: ""
screenshot_path: ""
...
extractor_diagnostics: {"readability_score":97,"text_length":12934,"paragraph_count":115,"sentence_count":79,"boilerplate_hits":0,"symbol_ratio":0.006,"replacement_count":0,"replacement_ratio":0,"control_count":0,"control_ratio":0,"binary_marker_count":0,"mojibake_marker_count":0,"binary_contaminated":false,"mojibake_contaminated":false,"method":"content-container"}
has_full_text: true
content_length: 12934
fetch_error: ""
...
discovery_record: {"discovery_title":"用 Genkit 构建智能体全栈应用","discovery_summary":"开源框架 Genkit 推出 Agents API，将消息历史、工具循环和流式传输封装为单一接口，简化对话 AI 开发。该 API 支持服务器或客户端管理的状态持久化，可实现历史分支、长时间运行的分离任务及多智能体协调等高级工作流，并通过统一线协议连接前后端。目前以 TypeScript 和 Go 预览版发布，集成 Genkit Developer UI，开发者无需编写客户端代码即可测试、调试和检查智能体快照。","source_name":"Google Developers Blog（RSS）","origin_url":"https://developers.googleblog.com/build-agentic-full-stack-apps-with-genkit","discovered_at":"2026-07-02T02:12:32.517Z","rank_on_page":141,"discovery_status":"discovered"}
source_role: resolved_original_source
origin_fetch_status: "success"
paywall_status: none
block_status: none
duplicate_status: unique
url_hash: 7e71677d07f1f183
content_hash: 5fe82e9cba6c76ab
semantic_hash: 2d628e3831af7bb0
duplicate_of: ""
first_seen_at: "2026-07-01T16:17:55.480Z"
last_seen_at: 2026-07-02T02:28:35.175Z
...
business_elements: {"companies":["Google Developers Blog（RSS）","Google","GitHub"],"products":["Genkit","Agents","copilot","agent","genkit","gemini","agents","Agent","Gemini"],"people":[],"industries":["开发者工具","企业服务"],"roles":["开发者 / 工程团队"],"workflows":["合同审阅 / 法律研究","部署 / 集成交付"],"business_actions":["发布 / 推出","部署 / 上线"],"affected_departments":["IT / 安全","销售 / 客服"],"numbers":["1","2026","10","123","8080","1000","5","2.0"],"quotes":["github.com/firebase/genkit/go/genkit/exp","assistant","googleai/gemini-flash-latest","You are a helpful assistant.","Hello. What can you do?"]}
...
Genkit solves this with the Agents API , which packages all of that behind one interface. You define an agent on the server, then drive it with the same chat() API whether it runs in process or behind an HTTP endpoint.
> The Agents API is in preview today in TypeScript and Go. It can introduce breaking changes in minor version releases.
Define an agent
...
The piece that ties your server and client together is the remote agent. remoteAgent() returns a handle with the same chat() interface as a local agent, so the code that drives an agent in your backend tests is the code that drives it from the browser. There is no separate request and response protocol to design, and no streaming format to invent.
...
When to reach for ADK instead
Genkit agents are an application primitive, built to live inside a full-stack, user-facing app. Consider the Agent Development Kit (ADK) instead when:
Multi-agent orchestration is the whole system, not just one feature. ADK is purpose-built for complex agent topologies, where Genkit's delegation middleware is deliberately lighter and not built into the core of the agent abstraction.
...
Head to the Full-stack agents documentation to dive in, or get started with Genkit if you are new to the framework. The API is in Beta, so we want your feedback: file an issue with what you build and what you would change.
Happy coding! 🚀
posted in:
AI
Announcements
...
- fetch_status: fetched-readable-text-content-container
- extraction_quality: high
- diagnostics: {"readability_score":97,"text_length":12934,"paragraph_count":115,"sentence_count":79,"boilerplate_hits":0,"symbol_ratio":0.006,"replacement_count":0,"replacement_ratio":0,"control_count":0,"control_ratio":0,"binary_marker_count":0,"mojibake_marker_count":0,"binary_contaminated":false,"mojibake_contaminated":false,"method":"content-container"}
...
2. **workflow_change**｜supports=signal_card_candidate, relationship_graph_input, business_change, case, trend_candidate_context｜importance=high｜confidence=high
   Build agentic full-stack apps with Genkit JULY 1, 2026 Chris Gill Product Manager Share Facebook Twitter LinkedIn Mail Announcing Genkit Agents: A full-stack foundation for conversational AI Genkit is an open-source framework for building full-stack, AI-powered and agentic applications for any platform with support for TypeScript, Go, Dart, and Python.

3. **case_detail**｜supports=signal_card_candidate, relationship_graph_input, case｜importance=high｜confidence=high
   Some of the most compelling AI features are conversational, like a support assistant that remembers the ticket or a copilot that works across several turns.
...
4. **company_action**｜supports=signal_card_candidate, relationship_graph_input, business_change, case, trend_candidate_context｜importance=high｜confidence=high
   Each needs more than a single generate() call, and building one today means wiring up message history, the tool loop, streaming, persistence, and a frontend protocol by hand.
...
5. **company_action**｜supports=signal_card_candidate, relationship_graph_input, business_change, case, trend_candidate_context｜importance=high｜confidence=high
   That plumbing repeats on every project and has little to do with what makes your app distinct.
...
6. **company_action**｜supports=signal_card_candidate, relationship_graph_input, business_change, case, trend_candidate_context｜importance=high｜confidence=high
   Genkit solves this with the Agents API , which packages all of that behind one interface.

## business_elements

- companies: Google Developers Blog（RSS）, Google, GitHub
- products: Genkit, Agents, copilot, agent, genkit, gemini, agents, Agent, Gemini
- people: 暂无公开信息
...
- numbers: 1, 2026, 10, 123, 8080, 1000, 5, 2.0
- quotes: github.com/firebase/genkit/go/genkit/exp / assistant / googleai/gemini-flash-latest / You are a helpful assistant. / Hello. What can you do?

## evidence_seed

- company_actions: Each needs more than a single generate() call, and building one today means wiring up message history, the tool loop, streaming, persistence, and a frontend protocol by hand. / That plumbing repeats on every project and has little to do with what makes your app distinct. / Genkit solves this with the Agents API , which packages all of that behind one interface.
- case_details: 开源框架 Genkit 推出 Agents API，将消息历史、工具循环和流式传输封装为单一接口，简化对话 AI 开发。该 API 支持服务器或客户端管理的状态持久化，可实现历史分支、长时间运行的分离任务及多智能体协调等高级工作流，并通过统一线协议连接前后端。目前以 TypeScript 和 Go 预览版发布，集成 Genkit Developer UI，开发者无需编写客户端代码即可测试、调试和检查智能体快照。 / Some of the most compelling AI features are conversational, like a support assistant that remembers the ticket or a copilot that works across several turns.
- workflow_changes: Build agentic full-stack apps with Genkit JULY 1, 2026 Chris Gill Product Manager Share Facebook Twitter LinkedIn Mail Announcing Genkit Agents: A full-stack foundation for conversational AI Genkit is an open-source framework for building full-stack, AI-powered and agentic applications for any platform with support for TypeScript, Go, Dart, and Python.
