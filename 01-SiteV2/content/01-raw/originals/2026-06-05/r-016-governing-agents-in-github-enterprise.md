...
original_url: "https://wellarchitected.github.com/library/governance/recommendations/governing-agents/"
canonical_url: "https://wellarchitected.github.com/library/governance/recommendations/governing-agents"
...
author: ""
published_at: ""
collected_at: 2026-06-05T00:46:23.091Z
language: mixed
full_text_hash: a4337c1bfad2f4ce
markdown_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-06-05/r-016-governing-agents-in-github-enterprise.md"
json_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-06-05/r-016-governing-agents-in-github-enterprise.json"
html_snapshot_path: ""
screenshot_path: ""
...
extractor_diagnostics: {"readability_score":97,"text_length":30331,"paragraph_count":210,"sentence_count":209,"boilerplate_hits":0,"symbol_ratio":0.0006,"method":"article"}
has_full_text: true
content_length: 30331
fetch_error: ""
...
url_hash: 03c873afc5afad1c
content_hash: 36b43d35336a282a
semantic_hash: 915f6febf461b4ec
duplicate_of: ""
first_seen_at: "2026-06-05T00:46:23.091Z"
last_seen_at: 2026-06-05T00:46:23.091Z
...
business_elements: {"companies":["keyword search","Anysearch","OpenAI","Anthropic","Microsoft","GitHub"],"products":["agents","Agents","Copilot","agent","Claude","Codex","MCP","Agent","copilot","AGENTS"],"people":[],"industries":["法律 / 法务","开发者工具","企业服务"],"roles":["开发者 / 工程团队","法务 / 律师"],"workflows":["合同审阅 / 法律研究","计费 / 预算管理","权限 / 安全治理","部署 / 集成交付"],"business_actions":["部署 / 上线"],"affected_departments":["IT / 安全","法务","财务 / 预算","销售 / 客服"],"numbers":["13","2026","6","1","2","3","4","5"],"quotes":[]}
...
What &ldquo;good&rdquo; looks like: agents operate within clearly defined trust boundaries, every action is auditable and traceable, model and tool access is centrally managed, and cost exposure is predictable and attributable.
Key design strategies and checklist
Minimize enterprise-level baseline, empower organizations to adapt
...
Enforce security controls and human review gates on agent output
The cloud agent&rsquo;s built-in protections provide a baseline, but they are not sufficient on their own. Layer additional protections including CODEOWNERS, branch rulesets requiring independent review, firewall restrictions, and least-privilege token scoping. Choose a code review agent strategy based on your team&rsquo;s risk tolerance.
...
Make agent activity auditable and traceable
...
Make cost exposure predictable and attributable
...
Enterprise custom agents exist in .github-private and are configured for enterprise-wide availability; org-scoped agents remain in the org&rsquo;s .github-private for narrower distribution; see enterprise custom agents
...
The GITHUB_TOKEN in copilot-setup-steps.yml is scoped to least privilege; note that this only applies to the setup workflow, not the agent&rsquo;s own operations
...
5. Cost management
Spending limits are configured per organization or cost center; &ldquo;stop usage at limit&rdquo; is enabled for hard caps; see manage agent-related costs
...
In most enterprises, start with &ldquo;Let organizations decide&rdquo; and enable for a pilot group first.
...
After configuring enterprise policies, communicate clearly to administrators and users. Policies only work if everyone understands them. If you choose &ldquo;Let orgs decide&rdquo; for agents, send guidance to org owners on how to enable agents within your enterprise&rsquo;s boundaries.
2. Layer agentic configurations
Consistent agent behavior requires layered configuration, and over-centralizing instructions and agents at the enterprise level hurts usability and increases cost. Prompts and custom instructions are most effective when tailored to the repository&rsquo;s language, framework, and domain. Enterprise-level controls should set security and compliance baselines; repository-level configuration is where teams optimize agent effectiveness for their specific codebase. In large enterprises with independent business units, pushing too much governance to the enterprise level leads to generic instructions, wasted tokens, and friction that slows adoption.
...
Agent-specific AGENTS.md (or CLAUDE.md , GEMINI.md ) See supported agent-specific file locations
...
Navigate to enterprise → AI controls → Custom agents section.
Select the source organization containing your .github-private repository.
Click Create ruleset in the &ldquo;Protect agent files using rulesets&rdquo; section.
...
Maintain an internal list of reviewed and approved MCP servers with their endpoints, allowed scopes, and review status.
Use rulesets to protect MCP configuration files ( .github/copilot/mcp.json or .mcp.json ) in repositories.
Configure an MCP registry to curate a discoverable set of pre-approved servers. If your policy requires strict control, set the MCP allowlist policy to &ldquo;Registry only.&rdquo; For enterprises where teams need to experiment, use &ldquo;Allow all&rdquo; with rulesets on .github/copilot/mcp.json and .mcp.json (step 2) so changes require review.
...
Customize the agent&rsquo;s development environment by creating a .github/workflows/copilot-setup-steps.yml in each repository. This workflow runs before the agent starts work, giving you deterministic control over the agent&rsquo;s environment. For runner selection, organization admins can set defaults centrally rather than configuring per-repository.
...
Cloud agent
Copilot cloud agent has built-in security protections . In the repository&rsquo;s settings, configure additional controls:
Firewall. The agent firewall is enabled by default, restricting the agent&rsquo;s internet access to an allowlist. Organization admins can manage firewall settings across all reposi
...
Agent changes to high-risk files (workflows, MCP config, agent instructions) git.push , pull_request.* with actor_is_agent High-risk modifications that could affect CI/CD pipelines or agent behavior. Correlate audit events with webhook payloads for file-level detail
...
Session transcripts give you what audit logs cannot: the agent&rsquo;s reasoning, the commands it ran, and the test output it saw before deciding what to do next. Use them for three things:
...
Actions minutes add up. Each agent session consumes GitHub Actions minutes for the duration of the agent&rsquo;s work. Monitor Actions usage to ensure agent workloads do not impact your CI/CD capacity.
...
Can&rsquo;t find what you&rsquo;re looking for? You can contact GitHub Support by opening a ticket .
GitHub Expert Services
GitHub&rsquo;s Expert Services Team is here to help you architect, implement, and optimize a solution that meets your unique needs. Contact us to learn more about how we can help you.
GitHub Partners
GitHub partners with the world&rsquo;s leading technology and service providers to help our customers achieve their end-to-end business objectives. Find a GitHub Partner that can help you with your specific needs here .
GitHub Community
...
For more details about GitHub&rsquo;s features and services, check out GitHub Documentation .
...
Essentials of governance and administration with GitHub Enterprise GitHub Enterprise Policies & Best Practices
...
- fetch_status: fetched-readable-text-article
- extraction_quality: high
- diagnostics: {"readability_score":97,"text_length":30331,"paragraph_count":210,"sentence_count":209,"boilerplate_hits":0,"symbol_ratio":0.0006,"method":"article"}
...
1. **case_detail**｜supports=daily_observation, heatmap, case｜importance=high｜confidence=high
   Agents — Copilot cloud agent, Copilot code review agent, and third-party agents like Anthropic Claude and OpenAI Codex— are becoming significant contributors to enterprise codebases. GitHub lets teams assign tasks to agents from issues, PRs, and Visual Studio Code in a single workflow. In many organ... / query=open-source AI agent GitHub enterprise adoption / intent=find_customer_case / path=developer_ecosystem

2. **case_detail**｜supports=daily_observation, heatmap, case｜importance=high｜confidence=high
   Content Library 📜 Governance Recommendations Governing agents in GitHub Enterprise Governing agents in GitHub Enterprise Kitty Chiu , Tiago Pascoal , Ken Muse , Josh Johanning & Ayodeji Ayodele April 13, 2026 Updated May 6, 2026 Scenario overview Agents — Copilot cloud agent , Copilot code review agent , and third-party agents like Anthropic Claude and OpenAI Codex — are becoming significant contributors to enterprise codebases.

3. **workflow_change**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   GitHub lets teams assign tasks to agents from issues, PRs, and Visual Studio Code in a single workflow.

4. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   In many organizations, agents already rank among the top contributors by pull request volume.

5. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   This introduces new governance challenges.

6. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=high｜confidence=high
   Agents act faster and at broader scale than any individual.

## business_elements

- companies: keyword search, Anysearch, OpenAI, Anthropic, Microsoft, GitHub
- products: agents, Agents, Copilot, agent, Claude, Codex, MCP, Agent, copilot, AGENTS
- people: 暂无公开信息
...
- numbers: 13, 2026, 6, 1, 2, 3, 4, 5
- quotes: 暂无公开信息

## evidence_seed

- company_actions: In many organizations, agents already rank among the top contributors by pull request volume. / This introduces new governance challenges. / Agents act faster and at broader scale than any individual.
- case_details: Agents — Copilot cloud agent, Copilot code review agent, and third-party agents like Anthropic Claude and OpenAI Codex— are becoming significant contributors to enterprise codebases. GitHub lets teams assign tasks to agents from issues, PRs, and Visual Studio Code in a single workflow. In many organ... / query=open-source AI agent GitHub enterprise adoption / intent=find_customer_case / path=developer_ecosystem / Content Library 📜 Governance Recommendations Governing agents in GitHub Enterprise Governing agents in GitHub Enterprise Kitty Chiu , Tiago Pascoal , Ken Muse , Josh Johanning & Ayodeji Ayodele April 13, 2026 Updated May 6, 2026 Scenario overview Agents — Copilot cloud agent , Copilot code review agent , and third-party agents like Anthropic Claude and OpenAI Codex — are becoming significant contributors to enterprise codebases.
- workflow_changes: GitHub lets teams assign tasks to agents from issues, PRs, and Visual Studio Code in a single workflow.
