...
json_snapshot_path: "01-SiteV2/content/01-raw/originals/2026-06-04/r-115-karpathy-的-llm-wiki-项目获超五千星.json"
html_snapshot_path: ""
screenshot_path: ""
...
business_elements: {"companies":["X","硅基流动 SiliconFlow (@SiliconFlowAI)"],"products":[],"people":[],"industries":["开发者工具"],"roles":[],"workflows":[],"business_actions":[],"affected_departments":[],"numbers":["5","000"],"quotes":[]}
...
4. **company_action**｜supports=daily_observation, heatmap, change, case, trend｜importance=medium｜confidence=medium
   让一个大语言模型构建并维护一个维基，每次使用时它都会变得更智能。
...
- company_actions: @karpathy 的 llm-wiki 在几周内获得了 5,000+ 颗星。 其理念是：停止在每个会话中重新发现知识。让一个大语言模型构建并维护一个维基，每次使用时它都会变得更智能。 以下是如何使用 @opencode + @justsisyphus OMO + SiliconFlow 构建你自己的版本 🧵 / @karpathy 的 llm-wiki 在几周内获得了 5,000+ 颗星。 / 其理念是：停止在每个会话中重新发现知识。
- case_details: 暂无公开信息
