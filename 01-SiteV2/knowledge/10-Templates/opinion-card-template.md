...
# 观点资产门禁：观点底稿 -> 独立栏目边界 -> 脚本质检
fact_draft_gate: pending
viewpoint_boundary_gate: pending
frontstage_regression_gate: pending

# 四档评级：feature | reference | archive | discard
...
# First-Line Viewpoints 展示字段。只有观点边界、原文证据和脚本质检通过后才可同步。
frontend:
  displayTitle:
  speakerLine:
  originalQuote:
...
|---|---|---|---|
| `feature` | `first_line_feature` | `frontstage_feature` | First-Line Viewpoints 主视图 |
| `reference` | `first_line_reference` | `frontstage_reference` | First-Line Viewpoints 参考视图 |
...
未回到原文的聚合标题 / 搜索摘要、`index_only` discovery artifacts 和 failed provider text 不能生成观点卡；只有回到原文且符合观点栏目规则的材料才能进入观点卡。
...
## First-Line Viewpoints 展示

仅在 `viewpoint_boundary_gate: passed`、`frontstage_regression_gate: passed` 且 `opinion_tier` 为 `feature` 或 `reference` 时填写或同步 `frontend` 对象。
