...
opportunity_signals:
  ...
  source_excerpt: "Claude Code v2.1.163 发布 Claude Code v2.1.163 发布 新增 `requiredMinimumVersion` 和 `requiredMaximumVersion` 托管设置，版本超范围时拒绝启动并引导用户使用经批准版本。新增 `/plugin list` 命令及 `--enabled`/`--disabled` 筛选；`/btw` 添加 \"c to copy\" 快捷键，复制原始 markdow..."
...
signal_owner: "Claude Code v2.1.163 发布"
...
谁：Claude Code v2.1.163 发布。

做了什么：Claude Code v2.1.163 发布 发布新的 AI 能力，面向企业智能体协作流程。

证据是什么：来源为 https://github.com/anthropics/claude-code/releases/tag/v2.1.163，对应本地证据 R-005，材料已保留全文、哈希和出处。
