---
id: SIG-20260609-A11
type: signal_card
signal_type: case
title: "rdjarbeng/yc-rfs-analysis-2026 · Datasets at Hugging Face"
date: 2026-06-09
status: published
source_title: "rdjarbeng/yc-rfs-analysis-2026 · Datasets at Hugging Face"
asset_level: frontstage
evidence_gate: core_evidence_passed
fact_draft_gate: passed
created_at: 2026-06-09T02:34:03.773Z
updated_at: 2026-06-09T02:34:03.773Z

raw_refs: ["R-061"]
pool_refs: ["P-044"]
primary_raw:
  raw_ref: R-061
  raw_archive: "01-SiteV2/content/01-raw/originals/2026-06-09/r-061-rdjarbeng-yc-rfs-analysis-2026-datasets-at-hugging-face.md"
  raw_json: "01-SiteV2/content/01-raw/originals/2026-06-09/r-061-rdjarbeng-yc-rfs-analysis-2026-datasets-at-hugging-face.json"
  source_url: "https://huggingface.co/datasets/rdjarbeng/yc-rfs-analysis-2026"
  full_text_hash: "24893d6f7f206f25"
  source_level: B
  extraction_quality: high
  has_full_text: true
  pool_routes:
    - core_pool
  raw_qc_decision: allow
  importance_type: important_vertical_solution
  importance_score: 5

formal_tags:
  track: ["track-ai-agent", "track-enterprise-workflow", "track-enterprise-data"]
  function: []
  scenario: ["scenario-knowledge-base"]
  customer: ["customer-enterprise"]
  evidence: ["evidence-customer-adoption"]
  stage: []
  region: []
  source: ["source-industry-data"]

opportunity_signals:
  schema_version: "opportunity-signals-v1"
  buyer_or_user: ["sales_team", "engineering_team", "enterprise_ai_owner"]
  team_or_function: ["sales", "engineering"]
  specific_task: ["sales_lead_research", "internal_tool_building"]
  business_action: ["funding_round", "research_benchmark"]
  product_form: ["vertical_saas"]
  delivery_model: []
  pain_or_constraint: []
  adoption_evidence: []
  source_evidence_type: ["funding_news", "research_paper"]
  evidence_basis: "raw_source_text"
  source_excerpt: "/ query=YC AI startup funding vertical AI / intent=find_startups / path=developer_ecosystem The dataset is currently empty."
  missing_fields: ["adoption_evidence", "delivery_model"]

signal_owner: "rdjarbeng/yc"

frontend:
  displayTitle: "rdjarbeng/yc-rfs-analysis-2026 · Datasets at Hugging Face"
  sourceLinks:
    - "https://huggingface.co/datasets/rdjarbeng/yc-rfs-analysis-2026"
---

# rdjarbeng/yc-rfs-analysis-2026 · Datasets at Hugging Face

## 新闻事实

rdjarbeng/yc-rfs-analysis-2026 · Datasets at Hugging Face

## 原文要点

- 原文未提供更多可拆分事实点，需以可见原文片段核对。

## 价值描述

rdjarbeng/yc 的案例信号可用于观察 AI 是否已经进入 销售和收入团队流程，以及后续是否出现客户、流程或结果指标。

## 可见原文片段

rdjarbeng/yc-rfs-analysis-2026 · Datasets at Hugging Face

## 证据边界

没有具体客户或真实企业案例
